...
4. **同步团队能力**：
   ...
   eat sync
   \`\`\`
   会把有权限的 Skill 落地到 \`~/.agents/skills/\`（并软链到 \`~/.claude/skills/\`），其中始终包含平台内置的 \`eat-platform-guide\`——之后的 AI 会话读它就知道 eat 平台有哪些能力、该怎么用（先查经验再求助、无权限走申请等）。同时输出团队 MCP 配置的合并指引。只想装进当前项目时用 \`eat sync --project\`（落 \`./.agents/skills/\` 并软链 \`./.claude/skills/\`）。
