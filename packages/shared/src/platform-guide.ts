...
3. \`get_help_request\` 看回复，\`reply_help_request\` 追问。求助内容只对求助双方与管理员可见。已解决的求助再回复默认会被重新打开（需要对方再处理时正该如此）；只是道谢或补充说明就带上 \`reopen=false\`（CLI: \`eat ask reply --no-reopen\`），求助保持已解决。
