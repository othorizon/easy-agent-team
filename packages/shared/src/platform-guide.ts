...
export const PLATFORM_GUIDE_VERSION = 11;
...
| \`eat skill list / export <slug>\` | 看团队里有哪些 skill / 把某个 skill 下载到本地目录（\`--out\` 指定落点）——想读它的完整内容、或以它为底改一份自己的时用 |
| \`eat skill push <dir>\` | 把本地写好的 skill 上传到平台纳管分享（改别人的 skill 要么你是作者，要么 \`--slug\` 换个名字推成自己的） |
