...
仍需要一个数据库和一次上线。eat 同样覆盖这一段：任何成员的 AI 都能按权限获得专属数据库账号，并将项目直接部署
上线，无需等待运维介入。
...
| 能力 | 说明 |
|---|---|
| 项目挂载 | 创建项目时可直接从 Dokploy 搜索并选择已有应用；Dokploy 未配置时降级为手动填写 |
| 成员管理 | 项目成员制，日志与部署权限收敛至项目成员，操作记入审计 |
| 部署门禁 | `eat deploy` 先在本地执行密钥扫描（通用规则 + **平台密钥指纹匹配** + `.env` 误提交检查），报告随部署请求上送，未通过则不予部署；另可选配一条本地预检命令，非零退出即中止部署 |
...
- **每个应用仅保留最近 10 条构建记录**，超出部分连同构建日志文件一并删除，且该数值在 Dokploy 中为硬编码、
  不可配置。这是部署历史默认只有 10 条的原因；更早的部署可通过 `eat project deployments <project> --all`
