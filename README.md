...
- 求助系统：Helper 登记 + 飞书机器人通知（支持加签）、双入口求助、多轮对话、平台 AI 整理经验
...
- CLI 平台自托管分发：一条命令安装（macOS/Linux `curl -fsSL <平台>/install.sh | sh`，Windows `irm <平台>/install.ps1 | iex`）；控制台安装页按系统分页签，并提供「给 AI Agent 的一键复制安装指令」
- 内置「平台使用指南」Skill：`eat sync` 对每个成员自动落地 `eat-platform-guide`，让本地 AI 认识平台能力与正确用法；Skill 统一落 `~/.agents/skills/` 并软链到 `~/.claude/skills/`（Windows 上改为复制实文件），`eat sync --project` 可改装到当前项目（`./.agents/skills/` + 相对软链 `./.claude/skills/`）
