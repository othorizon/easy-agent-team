...
清单按来源分「常规 / 数据库」两个页签（数据库分配生成的凭证环境归后者），可搜索、分页。
变量元数据（key、备注）默认全员可见，值需要授权；敏感值加密存储并打码（上图第二、三行，Owner / 管理员可点眼睛临时查看，读取照常记审计），
非敏感值对有权限者直接明文可见（第一行）。授权名单上方的 Owner 卡片说明 Owner 对整个环境拥有完整读写权限、不需要出现在名单里；
授权粒度是「用户 × 单个变量」或「用户 × 整个环境」，可以带有效期。
...
订阅人数对所有人可见；订阅者名单与「替某人订阅 / 取消订阅」只有管理员能看能做，
名单里会标明每个人是怎么订上的（手动订阅 / 角色模板 / 经验沉淀 / 捆绑）。
