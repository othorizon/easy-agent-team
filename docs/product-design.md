...
- **清单筛选与分页**：`GET /api/skills` 支持关键词 `q`（匹配 slug / 名称 / 触发描述）、`scope`（全部 / 已订阅 / 未订阅 / 我创建的）、`kind`（团队 / 私有 / 授予 / 捆绑 / 经验沉淀）与 `page` / `pageSize`（上限 1000），返回 `{ items, total, page, pageSize }`。控制台每页 20 条、可切 50 / 100 并支持跳页，筛选与页码同步到 URL；CLI `eat skill list` 默认取 100 条、`--limit` 最多 1000，配 `--search` / `--scope` / `--kind`。
