...
- **CLI / MCP 登录**：
  ...
  - **平台地址由 CLI 记住**（决策 59）：地址存在用户配置 `~/.eat/config.json` 的 `server` 里而不是只存在凭证里，`eat logout`、凭证过期或被手工删掉都不会把它带走，裸跑 `eat login` 仍连团队平台而不是悄悄回落到 `http://localhost:3000`；解析顺序为 `--server > EAT_SERVER > 已保存凭证 > 记住的地址 > 内置默认`，可用 `eat config get/set/unset server` 查改；
  - **跳转的平台地址自己处理，绝不把令牌交给别的主机**（决策 59）：`fetch` 在跨源跳转时会丢掉 `Authorization`（只换 scheme 也算跨源），所以 CLI 一律 `redirect: 'manual'`——同源跳转照跟，同主机的 `http → https` 跟随并把修正后的地址写回本地（自愈，只提示一次），换了主机则直接报 `PLATFORM_REDIRECT` 并给出该用的地址；
...
| **捆绑模式** | 管理员开启后，该 Skill 对所有成员恒为已订阅、不可退订，`eat sync` 总会同步；要求团队可见（§10 决策 37） |
...
1. **自建**：在平台上创建个性化 Skill（私有或分享）。**创建 / 推送不等于订阅**——作者对自己的 Skill 始终可见可改，但要不要让它进自己的同步范围，由自己订阅决定（§10 决策 34）。
...
4. **捆绑**：管理员把某个 Skill 设为捆绑，它对全体成员强制生效（§10 决策 37）——研发规范、安全红线这类「不该由个人决定装不装」的内容走这条路。

规则：

- 同步范围 =「角色模板内容 + 个人订阅 + 捆绑」（自建的 Skill 也要自己订阅一次才在其中），冲突时按 slug 去重、个人自建优先；
- 模板由管理员维护，模板更新后成员在下次 `eat sync` 时自动获得更新；
- 用户可在模板基础上**排除**个别不想要的条目；**捆绑的条目不可排除**（退订接口对成员直接返回 `SKILL_BUNDLED`），管理员自己不受捆绑约束、仍按自己的订阅记录算；
- **订阅人数**对所有人可见（口径 = 有效同步人数，只算启用中的用户）；**订阅者名单与代订阅仅管理员**：管理员可在 Skill 详情页看到每个订阅者的来源（手动 / 模板 / 经验沉淀 / 捆绑），并直接为某人订阅或取消订阅。
- **清单筛选与分页**：`GET /api/skills` 支持关键词 `q`（匹配 slug / 名称 / 触发描述）、`scope`（全部 / 已订阅 / 未订阅 / 我创建的）、`kind`（团队 / 私有 / 授予 / 捆绑 / 经验沉淀）与 `page` / `pageSize`（上限 1000），返回 `{ items, total, page, pageSize, counts }`——`counts` 是各 scope 下的条数（关键词与 kind 已应用、scope 自身不参与），供控制台的范围分段切换印在选项旁。控制台是「双行清单」（决策 38）：名称 + 触发描述（两行封顶、多行按原换行）+ 一行元信息，订阅按钮按状态分级（未订阅描边 / 已订阅灰底绿勾悬停变「退订」/ 捆绑只读锁），每页 20 条、可切 50 / 100 并支持跳页，筛选与页码同步到 URL；CLI `eat skill list` 默认取 100 条、`--limit` 最多 1000，配 `--search` / `--scope` / `--kind`。

### 3.2.3 同步机制（CLI）

- `eat sync`：将当前用户的有效 Skill 集合落地到本地，同时生成/更新 MCP 配置。安装范围三选一（互斥，类 npx skills 的 global/project 语义）：默认/`--global` 落 `~/.agents/skills/` 并逐个软链到 `~/.claude/skills/`（跨 Agent 工具共用一份）；`--project` 落当前项目 `./.agents/skills/` 并以**相对**软链接入 `./.claude/skills/`（仓库整体提交/移动后链接仍有效）；`--dir` 自定义目录，skill 直接落在该目录下，**不创建 `.agents`、也不碰 `.claude`**（给不遵循这套目录规范的 Agent 用）。历史直接落在 `.claude/skills/` 的受管目录会自动迁移为软链。**Windows 上把软链换成复制实文件**（§10 决策 24），其余语义（版本判断、冲突保护、清理）完全一致；
- **落点由配置记住**（§10 决策 56）：解析顺序为 `命令行 > 环境变量 EAT_SYNC_SCOPE/EAT_SYNC_DIR > 项目配置 ./.eat/config.json > 用户配置 ~/.eat/config.json > 内置默认`，显式带过作用域参数跑一次就自动写进配置（`--project` 写项目内配置，其余写用户配置；`--no-save` 跳过），之后裸跑 `eat sync` 落到同一处——更新提示里让 Agent 执行的正是裸命令。`eat config list/get/set/unset` 查改落点（只开 `sync.scope` / `sync.dir`，dir 一律存绝对路径）；`eat sync --dry-run` 只打印落点与将要变更的内容、不写任何文件。**上次装在别处、这次却回落到内置默认**（配置丢了/换了机器）时以退出码 1 中止并给出出路，配置给出的落点与预演都不拦；
- 采用「平台为准」的单向同步：本地被用户手工改过的沉淀目录会提示冲突，`--force` 覆盖；
- 每个落地的 skill 目录带 `.eat-meta.json` 记录来源与版本，便于增量更新与清理已退订项；
- **本地已有 skill 的纳管**：`eat skill push <目录>` 把本地写好的 skill 上传到平台——首次推送创建新 Skill（自己为 Owner），再次推送产生新版本；推送后平台成为该 Skill 的事实源，本地目录转为受管目录。**推送不改变订阅关系**（决策 34）：作者想让它随 `eat sync` 落到本地，自行 `eat skill subscribe <slug>` 一次（这条规则写在内置指南的命令速查表里；CLI 推送后**不再**逐次提示，见决策 40）。
- **控制台在线编辑正文**（决策 42）：`PUT /api/skills/:slug/content`，作者或管理员可在详情页直接改 SKILL.md，**保存即产生一个新版本**（与 `eat skill push` 同一套版本机制，订阅者下次 `eat sync` 拿到），版本历史里照常留一条带修改说明的记录。附属文件不在这个入口里改（原样沿用上一版），要增删附件仍走 `eat skill push`；编辑基于的版本号与服务端当前版本对不上时回 409，不覆盖别人刚推的版本；正文与当前版本完全一致时回 400，不灌水版本号。
- **元信息来自 SKILL.md frontmatter**：`eat skill push` 与网页创建的名称 / 触发描述默认取正文开头 YAML frontmatter 的 `name` / `description`（命令行 `--name` / `--description`、网页里已填的字段优先）。`description` 在社区 skill 里大量写成**块标量**（`>-` 折叠、`|` 保留换行）或引号包裹的跨行字符串，解析用 `packages/shared/src/frontmatter.ts` 的 YAML 子集解析器（三端共用）；服务端在客户端没给描述、或给上来的只是 `>-` 这类块标量指示符（旧版 CLI 的产物）时同样回退到正文 frontmatter（§10 决策 36）。**都读不到时保持平台上的原值、不覆盖**（§10 决策 44）：`name` / `description` 在推送请求里是可缺省字段，CLI 只在 `--name` / frontmatter 里真读到时才带上，**目录名只用来推 slug**；新建时没有原值可留，`name` 回落到 slug。
...
- 配置中的敏感字段（token）以**环境变量引用**方式声明，实际取值走环境变量的权限体系（见 3.3），实现一份权限模型管到底；
- `eat sync` 时按用户权限渲染出 `.mcp.json` 片段。

**凭证归属按传输方式收敛（§10 决策 52）**——两种传输各有唯一正确的通道，另一个字段对它毫无意义：

| 传输 | 凭证写在 | 另一个字段 |
|---|---|---|
| `stdio` | **`env`**（本地进程的环境变量） | `headers` 无处可用——stdio 根本不发 HTTP 请求 |
| `http` | **`headers`**（请求头） | `env` 会被 MCP 客户端忽略，填在这里等于静默失效 |

服务端 `upsert` 时按传输清空不适用的那个字段，`syncBundle` 也只渲染用得上的那个；控制台的编辑弹窗按传输切换，只显示该填的那一个。

**分发权限分两层（§10 决策 50）**——「谁看得到」与「谁用得上」分开：

```
可见性（配置属性，Owner / 管理员设置）
 ├── 团队可见：全员在清单里看得到名称与说明，可自助「申请订阅」
 └── 不公开：只有 Owner / 管理员看得到，成员只能由他们主动分配才拿得到
                （拿到之后清单里就看得到了——手上有什么必须心里有数）

订阅（用户与配置的关系）
 ├── 申请中（pending）：不进 eat sync 范围
 ├── 已批准（approved）：进 eat sync 范围
 ├── 已驳回（rejected）：不进，可重新申请
 └── 主动分配 / 角色模板：等同已批准，无需审批
```

- **订阅要审批**：成员点订阅 = 提一条申请（可写理由），由**配置 Owner 或管理员**批准，与环境变量的读取授权申请（§3.3.3）同一套心智——批准前 `eat sync` 拿不到这个配置。Owner / 管理员订阅自己能审批的配置即时生效，不必自己批自己；
- **审批入口在 MCP 配置页**：页头「订阅申请」按钮（有待审批时带状态圆点）弹窗里处理，默认只看待审批、可切「全部」回看已处理的；权限申请页（环境变量）只留一句指引链接，两类资源各管各的；
- **主动分配**：Owner / 管理员在配置的「订阅者」弹窗里直接把配置分配给成员（`source=admin`，等同已批准），不公开的配置只有这一条分发路径；角色模板（§3.2.4）派生的订阅同样无需审批——那本就是管理员定义的能力套餐；
- 审批、分配、收回都落审计（`mcp_config.subscribe_requested` / `subscription_decided` / `subscriber_added` / `subscriber_removed`）；
- **CLI / MCP 不开这个口子**：AI 不需要替用户申请或审批 MCP 配置的分发权（与§10 决策 41 / 45 同一条原则），订阅与审批只在控制台。

#### 分发形态：经网关分发 / 直连（§10 决策 51）

`http` 配置默认**经平台网关分发**，成员拿到的不再是真实配置，而是一条只属于自己的接入地址：

```
成员的 MCP 客户端
   │  https://<平台>/mcp/<slug>/<token>      ← 路径里的 token 即鉴权
   ▼
平台网关  ── 每次请求实时复算订阅是否仍生效
   │      ── 剥掉客户端送来的 Authorization，注入上游真实凭证
   │      ── 响应头白名单（www-authenticate / set-cookie / location 一律不回传）
   ▼
真实 MCP 服务（上游地址与凭证从不下发到成员本地）
```
...
- **长耗时调用不被网关掐断**（§10 决策 58）：`tools/call` 跑几十秒到几分钟是常态，网关等上游**响应头**的上限默认 3 分钟（`EAT_MCP_GATEWAY_UPSTREAM_TIMEOUT_MS` 可配），拿到响应头之后的流式传输不设限——该由客户端决定「等多久算太久」，代理不该替它决定。超时给明确的 `504 MCP_GATEWAY_UPSTREAM_TIMEOUT` 并落一条调用记录；客户端自己断开则就地释放上游连接、不记为故障。
...
环境（Environment）
 ...
 └── 变量（Variable）×N
      ...
      └── 未授权成员可见（默认开：只见变量名与备注、看不到值；关闭则完全隐藏，连存在都不知道。数据库分配生成的凭证环境例外，整组默认关——§10 决策 39）
```

- 环境示例：`公司内部服务`、`测试数据库`、`第三方 SaaS`；
- 环境有 Owner（创建者），Owner 和 Admin 可管理环境（编辑名称/备注、删除）与变量、授权；Owner 的权限不来自授权名单——控制台环境详情页在授权名单上方有一张 Owner 卡片把这层关系说清楚（§10 决策 39）；
- 环境有**来源**（`source`：手工创建 `manual` / 数据库分配生成 `db_assignment`），控制台清单按来源分「常规 / 数据库」两个页签，默认常规；清单接口 `GET /api/envs` 为分页信封 `{items,total,page,pageSize,counts}` + `q`/`source` 筛选，单个环境走 `GET /api/envs/:slug`（§10 决策 39）；
- 更新变量时 `value` 可缺省 = 保持当前值、版本不递增，只改备注 / 可见性 / 敏感标记——敏感变量的值控制台本就拿不到，不能逼着重填；有管理权者（Owner / 管理员）可在控制台点「眼睛」临时查看敏感值，走的就是正式拉取接口、照常落 `secret.read` 审计（§10 决策 39）；
...
|---|---|---|
| 已授权 | ✅ | ✅ |
| 未授权 + 变量开着「未授权成员可见」（默认） | ✅（明确标注"无权限"） | ❌，返回结构化错误并引导申请 |
| 未授权 + 变量关了「未授权成员可见」 | ❌（完全隐藏） | ❌ |
...
5. 全程记审计日志；
6. 申请记录永久保留（`access_request` 表：状态 / 审批人 / 审批时间 / 授权有效期，随环境或申请人删除而级联删除），审批人在控制台「历史审批」页签查看自己审批范围内已处理的申请——按审批时间倒序、分页、按结果筛选（§10 决策 45）。
...
3. 平台在实例上自动执行：建库 + 建账号 + 授权（也支持纯手工登记模式，兼容不能自动执行的场景）——PostgreSQL 建完库还要**把库里的 `public` schema 归到分配账号名下**，`CREATE DATABASE ... OWNER` 不管这件事（§10 决策 53）；
4. **分配结果自动生成为一个环境（或环境变量组）**：`DB_HOST / DB_PORT / DB_NAME / DB_USER / DB_PASSWORD`，Owner = 申请人、整组默认仅授权给申请人——其中**仅 `DB_PASSWORD` 为敏感**（加密存储、读取落审计），主机/端口/库名/账号为非敏感明文存储、有权限者平台明文可见（§10 决策 23）；整组变量默认**对未授权成员完全隐藏**（`visible_without_permission=false`，§10 决策 39）——这是某个人的项目库凭证，不是团队公共配置，别人的清单里不该出现；数据库账号的下发、拉取、审计**完全复用环境变量体系**；环境与分配记录**互相回指**（`db_assignment.environment_id` 反查得到环境的 `dbAssignment`），控制台环境详情页可跳到分配详情、分配详情页可跳到凭证环境；
5. **共用给其他成员 = 普通的环境授权**：Owner 可整环境授权他人（可设有效期），也可把变量改成「未授权成员可见」让 AI 经 `eat env list`/MCP 清单发现后自助申请；项目 skill 里可写明"连接信息从环境 xxx 拉取"作为引导，但凭证永远不进 skill 内容，只走环境变量通道；
...
- 沉淀产出的 SKILL.md 由平台统一在正文前合成标准 frontmatter（`name` = slug、`description` = 经验描述），保证落地本地后可被 AI 会话按 description 发现触发；写出的值经 YAML 标量转义（标题里带 `: ` 或以指示符开头也不会写出解析不了的 frontmatter，决策 36）；手工提供且自带 frontmatter 的内容不重复合成；
...
| 命令 | 功能 |
|---|---|
| `eat login [--server <地址>] [--no-wait] [--status] [--timeout <秒>]` / `eat logout` / `eat whoami` | 设备码登录（默认阻塞等待；`--no-wait` 发起后立即返回、`--status` 查一次并领取凭证，供 AI 自动执行，决策 54）、登出、查看当前身份；平台地址登录后被记住，`logout` 不会带走（决策 59） |
| `eat sync [--global\|--project\|--dir <dir>] [--dry-run] [--yes] [--no-save]` | 同步 Skill + MCP 配置到本地（模板 + 订阅 + 自建 + 沉淀经验）；落点按配置记住（决策 56），`--dir` 直接落指定目录、不创建 `.agents` / `.claude` |
| `eat config list \| get \| set \| unset` | 查改平台地址（`server`，决策 59）与 `eat sync` 的落点（`sync.scope` / `sync.dir`），决定裸跑的 `eat login` 连哪台、`eat sync` 装到哪 |
| `eat skill list [--search <词>] [--scope …] [--kind …] [--limit <n>] [--full]` / `eat skill subscribe / unsubscribe <slug>` | 浏览平台上可见的 skill 与订阅状态（默认 100 条、`--limit` 最多 1000；触发描述按显示宽度截断，`--full` 看全文；行首 `◆` 为捆绑）/ 订阅 / 退订（捆绑的退订返回 `SKILL_BUNDLED`，决策 37） |
...
| `eat env pull <env> [--out <f>] [--print] [--force]` | 拉取有权限的变量值，默认写入 `.env`；同名文件不是 eat 生成的则中止不覆盖，`--force` 覆盖前自动备份（决策 63） |
...
| Skill（捆绑） | 元数据+内容可见 | 管理员开启捆绑，全员强制订阅、成员不可退订 | Admin |
| Skill 订阅者名单 / 代订阅 | 只能看到订阅**人数** | — | Admin |
...
| 12 | `eat sync` 落地目录 | 实际文件落 `~/.agents/skills/`（跨 Agent 工具共用），逐个**软链**到 `~/.claude/skills/`；历史直接落地目录自动迁移；`--dir` 自定义时不建软链（§3.2.2；落点的记忆与漂移拦截见决策 56） |
...
| 33 | Dokploy 不出现在 CLI / MCP 面向 AI 的任何输出里 | **Dokploy 是 eat 平台的底层实现，CLI 与 MCP 是给 AI 用的，AI 既用不上也操作不了 Dokploy，把这个概念暴露给它没有意义。** 决策 31 之后 CLI 的帮助与输出、MCP 的工具描述与返回、内置指南、服务端透传的报错里到处都是「在 Dokploy 上建应用」「Dokploy 侧触发」「Dokploy application: <id>」，AI 读到后会以为要去那边做什么。收敛规则：① 面向 AI 的文字一律不提 Dokploy，需要指代时说「部署后台」，需要指代那台机器上的动作时直接说事（「创建应用」「触发部署」「构建记录已被清理」「绕过平台直接触发」）；② `AppInfo.dokployApplicationId` 仍在 API 里（控制台的挂载 / 编辑要用），但 CLI 不再打印、MCP 的 `list_apps` / `create_app` / `update_app` 返回前剥掉；③ 部署来源枚举 `dokploy` 改名 `external`（platform / external），错误码 `DOKPLOY_UNAVAILABLE` / `DOKPLOY_PROVISIONING_UNCONFIGURED` 改为 `DEPLOY_BACKEND_UNAVAILABLE` / `DEPLOY_BACKEND_UNCONFIGURED`，报错文案里给管理员的指引指向「系统设置 → 部署后台接入」，控制台该板块标题相应改为「部署后台接入（Dokploy）」——Dokploy 这个词只在管理员配置接入的那一块出现；④ 控制台其余部分与人看的文档（README、本文档、CLAUDE.md）照常说 Dokploy，那是给维护者与管理员看的。CLI 升 0.5.2、平台指南升 10（§3.7、§3.8） |
| 34 | 推送不再自动订阅 | **推送（`eat skill push` / 网页创建 / 经验沉淀共用的 `SkillsService.push()`）不建立任何订阅关系；订阅与否完全由用户自己决定。** 此前 push 末尾会把推送者自动订阅上去（注释写的是「保证出现在 sync 范围」），三个副作用：① **退订留不住**——退订自己的 skill 后推个新版本又被订回来（订阅行是物理删除的，`onConflictDoNothing` 拦不住重新插入）；② 管理员替他人推新版本时，被订阅的是管理员而不是作者；③ 经验沉淀得靠「先自动订上、再按 `grantedToHelper` 删掉」绕一圈才能表达「不沉淀给自己」。改动：① 删掉 push 里的订阅写入；② 经验沉淀改为按 `grantedToRequester` / `grantedToHelper` **各自显式插入**订阅，且不再删除未被勾选那方的既有订阅——「不沉淀给自己」是「不主动给你加一份」，不是「把你自己订过的退掉」；③ CLI push 后按返回的 `subscribed` 提示 `eat skill subscribe <slug>`，内置指南的速查表同步写明。**可见性与编辑权不受影响**：`canSee` / `canManage` 一直按 ownerId 判，变的只是「进不进 `eat sync` 范围」。顺带修掉一个测试假象：更新检测的指纹用例打的是 `@Public` 的 `/api/health` 读响应头，而 per-user 指纹在无 authUser 时根本不下发，读到的一直是空串（几条断言实为空串自比），改打需登录的 `/api/skills`。CLI 升 0.5.4、平台指南升 12（§3.2.2、§3.2.3、§3.6.1、§3.8） |
| 35 | 演示数据与截图 / 录屏工具进仓库，画面一律用真数据 | **README 的界面截图与命令行录屏由 `scripts/demo/` 下的脚本生成，且只允许拍真的东西。** 产品能力（元数据可见取值受控、求助与经验、部署门禁、绕过可见）光靠文字讲不清，需要图；而图一旦是拼的、数据是手塞进库的，就会与真实行为脱节且无法维护。定下的做法：① **演示数据经平台自己的 HTTP API 写入**（`seed-demo.mjs` / `seed-demo-apps.mjs`），不直接写库——版本号、审计、授权关系、真实建库、Dokploy 上的 application 与域名都由服务端逻辑产生；② **命令行录屏是把命令真跑一遍录下来**（`casts.mjs` 定义、`record-cli.mjs` 执行并记时间戳、`render-cast.mjs` + `gif.py` 渲成终端窗口样式的 GIF），输出一字不改，「另一个人在浏览器里点了同意 / 批了申请 / 回了求助」这类旁人动作由录制器在命令执行中途真调接口完成；演示成员的 HOME 与工作目录用真实路径（`/home/<用户>/work/...`），命令输出里的路径就是它本来的样子；③ **截图用 Playwright**（`capture-web.mjs`），2 倍分辨率拍完缩到 1440 宽（超采样，字更干净、体积更小），窗口高度按内容自适应避免大片留白；④ 产物入库：截图 `docs/assets/screenshots/`、GIF `docs/assets/demos/`（六段共 ~740KB），全集页 `docs/screenshots.md`，重跑步骤与踩过的坑记在 `scripts/demo/README.md`（演示 Git 服务必须是 smart HTTP、基础镜像要先 `docker pull`、录屏有先后依赖、截图放最后拍）。**顺带修掉录屏时暴露的两个 CLI 问题**（CLI 升 0.5.5）：`eat env pull <环境>` 不带 `--keys` 且一个变量都没权限时，服务端只回「有权限的那些」故 values / denied 双空，CLI 什么都不打印就退出——与「无权限要给可执行的申请引导，而非静默失败」的原则相悖，改为回落到清单并列出该申请的 key；密钥扫描的候选 token 正则把 `=` 放进了字符集，导致 `KEY=<密钥>` 这种**最常见的泄漏形式**被整行吞成一个 token、指纹永远匹配不上，改为 `=` 只允许作为结尾的 base64 padding（补了赋值形式与 base64 padding 两个单测）。 |
| 36 | SKILL.md frontmatter 按 YAML 子集解析，三端共用 | **`description: >-` 这类块标量写法必须能被正确解析——此前 CLI 里那版「一行一个 `key: value`」的正则只能取到 `>-` 这个指示符本身，推上平台的描述就成了 `>-`，AI 因此完全无从判断何时该用这个 skill。** 描述是 Agent Skill 被发现的唯一依据，解析错等于 skill 报废。改动：① 解析器提到 `packages/shared/src/frontmatter.ts`，CLI / 服务端 / 控制台共用一份，覆盖块标量（`>`、`|`，含 `-`/`+` 截断与缩进数字）、单双引号标量（`''` 与反斜杠转义，均可跨行）、纯标量的缩进续行、CRLF、BOM、整行注释、`...` 结束分隔符；**嵌套结构的值（如 `allowed-tools:` 下的序列）跳过而不是当标量吞掉**，行内 `#` 不当注释剥离（描述里出现 `#` 比写注释常见得多）。不引入 YAML 依赖：CLI 是 tsup 单文件分发，为两个字段拖进一个解析库不划算。② **服务端也解析一次**：`SkillsService.push()` 在客户端没给名称 / 描述（网页只贴了正文没填描述），或给上来的只是 `>-`、`|2-` 这类块标量指示符（旧版 CLI ≤0.5.5 的产物）时，回退到正文 frontmatter——已装旧 CLI 的用户不必先升级，推一次新版本描述就自动修好。显式给了值的以客户端为准。③ **控制台创建 Skill 支持直接粘贴整份 SKILL.md**：正文失焦时从 frontmatter 回填**还空着**的标识 / 名称 / 触发描述（已填的不覆盖），并 toast 说明填了哪几项。④ 经验沉淀合成 frontmatter 时改用 `toYamlScalar()` 写值，求助标题里出现 `: `、以 `#` 开头之类不再写出解析不了的 frontmatter；「已自带 frontmatter 就不合成」的判断也改走同一个解析器（容忍 BOM 与 CRLF）。⑤ **多行描述的展示端一并处理**：`|` 保留块解析出来的描述是真多行（社区 skill 里 `description` 写成一份带列表的触发规则很常见），HTML 会把换行折成空格，控制台详情页因此把一整份触发规则挤成一坨——详情页的触发描述改 `whitespace-pre-line` 按原样换行（并 `break-words` 防长串溢出），编辑元信息的输入框从 2 行放到 4 行；Skill 列表页本来就是 `truncate` 单行截断，不用改。CLI 的**一行一条**清单（`eat skill list`、`eat ask targets`）改用 `oneLine()` 把描述折成单行、并按**显示列宽**截断到 200 列（一个汉字算两列——按字符数截会漏掉这点，同一个数字对中文等于放宽一倍），否则一条 skill 就能刷满屏幕、把「（作者: xx）」挤到十几行之后。200 列 ≈ 100 个汉字，实测团队 skill 的触发描述绝大多数在这个长度内完整显示，只有「把整套触发规则写进 description」的那种（官方 learn 那类近千字符）会被截；`eat skill list --full` 不截断，另有 `eat skill export` 与控制台详情页看全文，且截断发生时清单末尾会提示这两条路。⑥ **存量数据要订正**：服务端的回退只在 push 时生效，**已经躺在库里的坏描述不会自己变好**——线上先有的 skill 一直显示 `>-`，而描述是 AI 判断何时用这个 skill 的唯一依据，坏着等于这些 skill 是废的。故在 `pnpm db:migrate`（容器 entrypoint 每次启动都会跑）之后加一步幂等的数据订正 `repairSkillDescriptions()`：扫出描述为空或只是块标量指示符的行，按其**当前版本的 SKILL.md 正文**重新解析，解析不出就保持原样；不动 `updated_at`（这是订正不是更新，别把 skill 顶到列表最前，也别触发「Skill 有变更」的错觉）。重新部署即自动修好，不需要用户逐个重推。⑦ 验证：20 个解析单测（各种块标量 / 引号 / 续行 / CRLF / BOM / 嵌套跳过 / 生成值往返）+ 5 个服务端 e2e（网页空描述回退、旧 CLI `>-` 回退、显式值优先、推新版本时描述跟着更新、存量坏行订正且幂等）；真机冒烟——`eat skill push` 推一个 `description: >-` 的 skill，`eat skill list` 打出折叠后的描述；把官方 `learn` skill（`description: |` 的多行触发规则）推上去看控制台详情页按原样换行、CLI 清单仍是一行一条；控制台粘贴整份 SKILL.md 后回填并落库，桌面 / 移动双视口 Playwright 验证。CLI 升 0.5.7（§3.2.3、§3.6.1） |
| 37 | Skill 捆绑模式 + 清单筛选分页 + 订阅者管理 | **管理员可把一个 Skill 设为「捆绑」：它对所有成员恒为已订阅、不可退订，`eat sync` 总会同步；管理员自己是例外，仍按自己的订阅记录算。** 研发规范、安全红线这类内容不该由个人决定装不装，此前只能靠角色模板（用户可以「排除」掉）或挨个催人订阅。① **落库**：`skill.bundled`（迁移 `0012_skill_bundled`）。**要求 `visibility='team'`**——否则会出现「被强制订阅却看不到内容」的自相矛盾状态，故开启捆绑时校验可见性、捆绑期间也不许改成私有（两个方向都在 `updateMeta` 里按**结果状态**判，不是按入参判）；`canSee` 里也补了 `bundled → true` 兜底。**只有管理员能改这个开关，作者对自己的 Skill 也不行**（这是替全员做决定）。② **有效同步集合**从「订阅 ∪（模板 − 排除）」变成「订阅 ∪（模板 − 排除）∪ 捆绑」，捆绑对非管理员**连排除标记也压过去**；`SyncSkill.relation` 加 `bundled`（优先级 own > bundled > subscribed > template），成员退订捆绑 skill 直接回 400 `SKILL_BUNDLED`（**订阅仍照常写记录**——万一之后取消捆绑，用户自己订过的那份要留着）。指纹（决策 26）自动跟着变，管理员开捆绑后成员下次执行任何 eat 命令都会收到「团队 Skill 有变更」。③ **清单改为服务端分页 + 筛选**：`GET /api/skills` 返回 `{items, total, page, pageSize}`（**破坏性变更，照决策 30 的先例不留兼容层**），支持 `q`（slug / 名称 / 触发描述，大小写不敏感）、`scope`（all / subscribed / unsubscribed / mine）、`kind`（all / team / private / granted / bundled / experience）、`page` / `pageSize`（上限 1000）。可见性过滤依赖当前用户的订阅集，仍在应用层判，故是「取全表 → 过滤 → 切片」，**订阅人数只为当前页那几条算**。控制台每页 20 条、可切 50 / 100、页码 + 跳页，筛选与页码同步到 URL（`lib/use-query-params.ts`，等于默认值的不写进 URL），搜索框 300ms 防抖，翻页用 `keepPreviousData` 不闪骨架屏；CLI 按 gh 的做法**默认 100 条、`--limit` 最多 1000、不翻页**（AI 拿到半页会当成全部去下结论），配 `--search` / `--scope` / `--kind`，取不全时末尾提示还有多少条。④ **订阅人数与订阅者管理**：`SkillInfo` 加 `bundled` / `subscriberCount` / `subscriptionLocked`（后者供前端禁用按钮）。人数口径 = 有效同步人数（手动 / 经验订阅 ∪ 模板 − 排除 ∪ 捆绑覆盖的成员），**只算启用中的用户**（禁用的人登录不了也不会 sync，算进去会让「多少人在用」失真），人数与明细走同一个 `effectiveSubscribers()`，两边口径不会漂。**人数全员可见，名单与代订阅仅管理员**：`GET/POST /api/skills/:slug/subscribers`、`DELETE …/subscribers/:userId`（`@Roles('admin')`），明细带来源（手动 / 模板 / 经验沉淀 / 捆绑）与 `removable`；代订阅拒绝私有 skill（对方看不到）与禁用用户，捆绑 skill 拒绝单独增减（要先取消捆绑）；模板派生的移除仍写「排除」标记而不是删模板，自助订阅与代订阅共用同一份写入逻辑。⑤ **UI 按操作频次分层**：列表页只放搜索 / 筛选 / 订阅这些高频操作，捆绑开关收进详情页的「编辑元信息」弹窗（管理员才渲染），订阅者名单与「添加订阅者」收在详情页下方的管理员专属卡片里。⑥ 顺带修两处：详情页的 `subscribed` 此前只看手动订阅、**模板派生的 skill 在清单里显示已订阅、在详情页却显示未订阅**，改为与清单同一个有效集合；窄屏下 Skill 列表第一列的描述是 `truncate`（即 `nowrap`），不封顶会把表撑到要横向滚动、把「订阅」按钮挤出屏幕，加 `max-w-[55vw]`。⑦ 验证：server e2e 从 146 涨到 174 条（分页信封 / 翻页并集 / 越界页 / 关键词 / scope / kind / pageSize 上限与非法枚举、捆绑的权限与可见性约束、成员退订被拒、管理员例外、取消捆绑后回落、人数口径含禁用用户、代订阅的四类拒绝），CLI 单测补 `--limit` / 查询串拼装 / 行首标记；真机全链路冒烟——管理员设捆绑 → 成员 `eat skill list` 见 `◆`、`unsubscribe` 被拒、`eat sync` 落地 → 取消捆绑后 sync 移除 → 管理员代订阅 / 代取消，成员 sync 随之增删；控制台桌面 1440 与移动 390 双视口 Playwright 走查（搜索 / 筛选 / 跳页 / 刷新还原 / 捆绑徽标 / 禁用按钮 / 订阅者卡片 / 添加订阅者弹窗，无横向溢出、无控制台报错）。CLI 升 0.5.8、平台指南升 13，迁移 `0012_skill_bundled`（§3.2.1、§3.2.2、§3.8、§4、§5.2） |
| 38 | Skill 列表页重设计为「双行清单」，范围筛选带数量 | **列表页的主要动作是「读触发描述 → 决定订不订」，原表格恰恰把这件事做得最差：slug 做成代码块压在名称上方、机器名盖过人读的名字；触发描述固定单行截断，长描述看不到重点、短描述留一片空白；每行一个实心黑「订阅」按钮，20 行就是 20 个视觉重心；已订阅 / 未订阅只靠按钮文字区分，没有一眼可辨的状态。** 拿同一组 8 条样例（已订阅 / 未订阅 / 捆绑 / 私有 / 授予可见 / 经验沉淀 / 我创建的 / 悬停 / 进行中 + 极短名 / 超长名 / 空描述 / 超长描述 / 多行描述）对比了双行清单、卡片网格、紧凑表格三套方案后选定双行清单：卡片一屏只放 9 张且长名只能一行截断，表格改开关容易误触、窄屏必须横向滚动。① **行的结构**：第一行名称（15px 半粗、整行链接、允许换行不截断）+ slug 退为等宽小字 + 只出非默认状态的徽标（捆绑 / 私有 / 授予可见 / 经验沉淀——「团队可见」是默认值、九成行都是它，每行挂一个纯属噪音）；第二行触发描述 `line-clamp-2` 两行封顶、`whitespace-pre-line` 让 `\|` 块的多行描述按原换行显示（决策 36 说列表页「本就是单行截断不用改」，这次一并补上）；空描述用占位文案「未填写触发描述」替代空白，作者本人看到时追加「AI 无法据此判断何时使用它，建议补上」；第三行元信息压成一行小字——头像首字 + 作者（自己的显示加深的「我」：「我创建的」不值得一个徽标，但扫描时得能一眼挑出来）· vN · 订阅人数 · 相对更新时间（`apps/web/src/lib/format.ts`，悬停给完整时间）。② **订阅按钮按状态分级**，页面上不再有一列实心黑按钮：未订阅 = 描边（唯一需要用户动手的）；已订阅 = 安静灰底 + 绿勾，悬停整行时原位变红字描边「退订」——Tailwind v4 的 `hover` / `group-hover` 变体自带 `hover:hover` 媒体查询，触屏上按钮一直写「已订阅」、点它退订，与「Following」类按钮的惯例一致，`aria-label` 写「退订」；捆绑 = 只读的锁「已捆绑」（禁用但留着，比藏起来少让人困惑）；进行中 = 加载态防重复点。③ **范围筛选从下拉改成带数量的分段切换**（全部 / 已订阅 / 未订阅 / 我创建的）：数量回答「切过去会有几条」，已订阅那个数就是 `eat sync` 会落地的条数。为此 `GET /api/skills` 的信封加 `counts {all, subscribed, unsubscribed, mine}`——在可见性 / 关键词 / kind 收窄后的集合上数、**scope 自己不参与**，所以切范围时 `total` 变、`counts` 不变，分段切换上的数字才稳得住；加字段不是破坏性变更，CLI 只读 `items` / `total` 不受影响。类型是低频筛选，留在下拉。④ **去掉外层 Card 与表格**：行之间用分隔线，悬停整行淡灰底 + 名称下划线；窄屏（<640px）slug 从名称行挪进元信息行、去掉头像与更新时间，订阅按钮挪到元信息行右端；加载骨架换成对应三行结构的 `ListSkeleton`。⑤ 验证：server e2e 加 counts 用例（关键词参与、scope 不参与、kind 参与），175 条全过；真机 Playwright 桌面 1440 / 移动 390 双视口走查——管理员与成员两种视角，上述全部状态与内容长度，悬停换字、订阅 → 退订 → 订阅往返、范围切换与搜索同步到 URL，无横向溢出、无控制台报错；`docs/assets/screenshots/skills.png` 与 `mobile-nav.png` 用演示数据重拍。不改 CLI 与内置指南，版本号不动（§3.2.2） |
| 39 | 数据库分配详情页 + 凭证环境与分配记录互相回指 + 凭证变量默认隐藏 + 环境清单分页筛选与 Owner 卡片 | **用户视角的几个断点一起补**：① 「全部分配」清单里一行放不下的东西（实例地址、账号、处理人与时间、失败原因、凭证在哪）没有地方看——新增分配详情页 `/db/:id`（`GET /api/db/assignments/:id`，申请人或管理员可看，已删除的记录直接访问仍可见、只是不再进清单），管理员操作按钮（批准 / 驳回 / 禁用 / 恢复 / 删除）与清单页共用 `apps/web/src/pages/db-shared.tsx`。② 凭证环境与分配记录之间**看不出关系**：关系其实一直在 `db_assignment.environment_id` 上，只是没暴露——`EnvironmentInfo` 加 `source`（`manual` / `db_assignment`）与 `dbAssignment {id, dbName, instanceName, status}`（反查一次拿整张映射，分配记录已不存在时为 null），`DbAssignmentInfo` 加 `environmentId` / `instanceHost` / `instancePort` / `decidedByName` / `updatedAt`；环境详情页头下一行「数据库分配自动生成 · 库 · 实例 · 状态 · 查看分配」，分配详情页「连接凭证」卡片直达环境并给出 `eat env pull` 命令；不加新列、不做迁移。③ 凭证变量**默认对未授权成员完全隐藏**（建变量时 `visible_without_permission=false`）：这是某个人的项目库凭证，不是团队公共配置，出现在别人的清单里只会引来无意义的申请；Owner 想共享时授权整个环境即可，也可以手动打开可见。存量凭证环境不动。④ 环境清单页照 Skill 清单（决策 38）重做：`GET /api/envs` **破坏性地**改成分页信封 `{items,total,page,pageSize,counts}` + `q`（匹配 slug / 名称 / 备注 / 关联库名）/ `source` / `page` / `pageSize`（默认 20、上限 1000，照决策 30 不留兼容层；控制台、e2e、演示种子同步改；CLI / MCP 只用 `/api/catalog`，其内部改走不分页的 `loadEnvironments()`，不受影响）；新增 `GET /api/envs/:slug` 给详情页与选择器用——清单已分页，靠翻清单找一条不可靠；页签「常规 / 数据库」默认常规、带数量（关键词已应用、source 不参与计数，与决策 38 的 scope 计数同一思路），搜索 300ms 节流落 URL，分页复用 `components/pagination.tsx`；行结构：名称 + slug 小字 → 备注两行封顶 → 元信息（Owner「我」· 相对创建时间），右侧只放变量数；数据库页签的行多一行「库 · 实例 · 状态 · 分配详情」。⑤ **更新变量时值可缺省**（`upsertVariableSchema.value` 改 optional）：此前想给敏感变量改可见性会因为值必填而改不了，而敏感值控制台本就拿不到。缺省 = 保持当前值、**版本不递增**（版本只跟着值走）；翻转敏感标记时把现值解出来搬到另一列；新增变量缺省值仍拒（400）。控制台更新框的值不再必填、留空即保持，非敏感变量带出的旧值没改也按没改处理。⑥ **有管理权者可在控制台查看敏感值**：值列打码旁加「眼睛」，点了走正式的 `POST /api/envs/:slug/values` 拉取——服务端照常落 `secret.read` 审计，与终端 `eat env pull` 一视同仁，不另开后门；限 Owner / 管理员（与「更新」按钮同一批人），被授权的普通成员仍走 CLI / MCP 拉取。⑦ 环境详情页在「读取授权」上方加 **Owner 卡片**：头像首字 + 姓名 + 邮箱 + 「环境 Owner」徽标，一句话说清「Owner 对整个环境拥有完整的读写权限，不需要出现在授权名单里；管理员同等」，下面三条能力勾选（读全部值含敏感值 / 增删改变量 / 授予撤销授权）；数据库凭证环境补一句「申请人即 Owner」，无管理权者补一句「怎么申请」，管理员非 Owner 时补一句「你以管理员身份查看」。为此 `EnvironmentInfo` 加 `ownerEmail`（用户清单本就对登录用户开放 email）。顺带：删除数据库凭证环境的确认文案改成提醒「密码无法找回，回收库请去数据库页删分配记录」。不涉及 CLI 与内置指南，版本号不动。 |
| 40 | `eat skill push` 不再提示「你还没订阅它」 | **推送成功后只报推送结果，不再逐次劝订阅。** 决策 34 给 CLI 加的那句「你自己还没订阅它（推送不会自动订阅）。想让它随 `eat sync` 落到本地: `eat skill subscribe <slug>`」，对**刻意不订阅自己 skill** 的作者（写给别人用、或本地已有工作副本）每推一次就重复一次，属于噪音；「推送不建立订阅」这条规则本身留在内置指南的命令速查表里（`eat skill push` 那行）与本文 §3.2.2，不靠每次推送重复。服务端 `POST /api/skills/push` 返回体里的 `subscribed` 字段不动（`SkillDetail` 通用字段，控制台也在用）。CLI 升 0.5.9；内置指南内容未变，版本不动（§3.2.2） |
| 41 | 求助对话可以删掉自己写的那条回复 | **回复发出去之后要能撤回**：手滑发半句、贴错内容、把密钥贴进了对话，此前唯一的补救是把**整条求助**删掉（还得是求助者本人、且没沉淀过经验），代价太大。新增 `DELETE /api/help-requests/:id/messages/:messageId`，**回复者本人或管理员**可删（与「删除求助」同一套「本人或管理员」口径；管理员本就能删整条求助、这里只是给了粒度更细的手段，且求助已沉淀时整条删不掉、单条还能删）。两个设计点：① **状态按剩余消息回推**——删掉被求助者的唯一回复后不该还挂着「已回复」，规则是「最后一条来自被求助者 → `answered`，否则 → `open`（一条不剩也回 `open`）」，而 `resolved` / `closed` 是人工拍板的终态、不因删消息回退；② 只在**控制台**开口子（用户原话「网站上应该支持」），CLI / MCP 不加命令——AI 侧误发的补救仍是 `delete_help_request` 删整条，避免给 AI 一个悄悄抹掉自己发言的工具。删除不推 webhook（对方已经收到过通知，再推一条「我删了」是噪音），落审计 `help.reply_deleted`（只记 messageId / senderId，不抄正文）。路径参数在进 uuid 列比较前用 `isFullId()` 挡一道，乱写的 messageId 回 404 而不是 500。不涉及 CLI 与内置指南，版本号不动（§3.5.3） |
| 42 | 控制台可在线编辑 SKILL.md，保存即新版本 | **改一句话不该必须回到有 CLI 的机器上**：此前 Skill 正文只能 `eat skill push` 推，网页端只能改元信息（名称 / 描述 / 可见性 / 捆绑）与看只读正文。新增 `PUT /api/skills/:slug/content`，详情页 SKILL.md 那块可切换成编辑态，**保存 = `currentVersion + 1`**，走的就是 `skill_versions` 既有的版本机制（无迁移、无新表），订阅者下次 `eat sync` 自然拿到。四个设计点：① **权限是「作者或管理员」**，与 `PATCH /api/skills/:slug`（元信息）和删除同一口径——作者本就能用 CLI 推新版本，不必在网页上被挡；② **附属文件不在这个入口里改**，原样从上一版沿用（这个入口只传正文，传空会把附件悄悄删光），增删附件仍走 `eat skill push`；③ **乐观并发**：请求带编辑时看到的 `baseVersion`，与服务端当前版本对不上回 409 并报出实际版本，不覆盖别人刚推的；④ **正文没变就不产生新版本**（400），避免误点保存把版本号灌成一串一模一样的内容。正文的 frontmatter 仍是元信息事实源（与 push 一致）：写了 `name` / `description` 就跟着更新，没写则保持原值。大小限制与密钥扫描复用 push 的 `validatePayload()`（参数类型放宽成 `{content, files}`，调用方不变）。审计仍记 `skill.version_pushed`，meta 加 `via: 'console'`，让版本来源可分辨。不涉及 CLI / MCP / 内置指南，版本号不动（§3.2.3） |
| 43 | 求助清单加「待回复 / 全部」筛选，与页签正交 | **求助页此前只有「找我的 / 我发起的」一个维度，已解决、已关闭的历史和当下要处理的混在同一张表里**，人越用越难扫。加一维**进展**筛选，两个页签共用同一个值、默认选「待回复」。① **口径**：「待回复」= 对话**还等着某一方回应**——`open` 等被求助者回复、`answered` 等求助者确认或追问；相对的 `resolved` / `closed` 是不再需要任何人回应的终态。刻意**没有**把 `answered` 排除在外：页签角标「我发起的」数的正是 `answered`（有新回复待我确认），「待回复」若只认 `open`，就会出现「角标说有 2 条、点进去一条也看不到」的自相矛盾。② **两个维度必须长得不一样**，否则会被当成同一组开关（`TabsList` 与原来的 `Segmented` 恰好是同一套实心轨道样式）：页签保持实心轨道（`bg-muted` + 白色滑块 = 导航），筛选改用新加的 `Segmented variant="outline"`（描边轨道 + 灰色滑块 = 筛选）；桌面同处一行、页签靠左筛选靠右与卡片右缘对齐，窄屏换行堆叠。③ **两组数字各司其职**：页签角标 = 需要我出手的条数（找我的：等我回复；我发起的：有新回复待我确认），分段上的小字 = 「切过去会有几条」（按**当前页签**算，与决策 38 的 `counts` 同一口径），所以切页签数字变、切筛选数字不变。④ **空状态分两种**：全被筛掉时说「没有待回复的求助，N 条已了结的记录都在「全部」里」并给「查看全部」按钮（给出口），真的一条都没有时才讲怎么开始（讲入门）。⑤ 筛选与页签都同步到 URL（`?status=` / `?tab=`，等于默认值的不写进去），本页原来的 `useTabParam` 换成 `lib/use-query-params.ts`，两个参数一套 hook。⑥ **纯前端过滤**：`/api/help-requests/inbox` 与 `/mine` 本就一次取 100 条，不动服务端、不加接口参数，CLI / MCP 与内置指南都不涉及，版本号不动。⑦ 演示数据补一条「吴敏 → 孙浩」的待回复求助（否则默认筛选下 `help.png` 是空的），`capture-web.mjs` 的 help-detail / mobile-help-detail 改挂 `?status=all` 才点得到那条已解决的，`docs/assets/screenshots/help.png` 重拍；桌面 1440 / 移动 390 双视口 Playwright 走查（默认筛选、切筛选、切页签保持筛选、深链还原、筛空的出口按钮，无横向溢出、无控制台报错）（§3.5.3） |
| 44 | `eat skill push` 不传 name / description 时保持平台上的原值 | **「不传」不是「清空」，是「这次没要改它」。** 此前 CLI 的元信息解析是 `--name → frontmatter → 目录名`，服务端 push 则无条件把请求里的 `name` / `description` 写回 `skill` 表；于是一个没写 frontmatter 的目录每推一次新版本，就把作者在控制台起的「运营周报生成」覆盖成目录名 `weekly-report`，触发描述则被抹成空——从用户视角就是「推送把原来的名字删了」。控制台在线编辑（决策 42）本就是「frontmatter 写了就更新、没写保持原值」，push 这条路只是没对齐。改动四处：① 契约 `pushSkillSchema` 的 `name` / `description` 改为**可缺省**（`description` 去掉 `.default('')`）；② CLI `resolvePushMeta()` 只在 `--name` / frontmatter 里真读到时才带上这两个字段，读不到就不放进请求体，**目录名只用来推 slug**（frontmatter 里写了空值等同没写——带上去只会被服务端的 `min(1)` 拒掉）；③ 服务端 `resolveMeta()` 改成取 `客户端传的 → 正文 frontmatter → 平台上的原值` 里第一个**可用**的（「不可用」= 没传 / 空串 / 块标量指示符，沿用决策 36 的判定），为此把既有行的查询提到解析之前；新建时没有原值可留，`name` 回落到 slug、`description` 回落到空串；④ CLI 在读不到时各打一行提示（点明「已有的保持原值、新建的以 slug 作名称」），并在推送成功那行补印平台上的名称，让「名字没被改掉」当场看得见。网页创建的 name 仍是必填，不受影响；MCP 没有推送工具，不涉及。CLI 升 0.5.10；内置指南内容未变、版本不动（§3.2.3） |
| 45 | 权限申请页加「历史审批」页签，服务端新增历史清单接口 | **申请记录一直都在，缺的是看它的入口。** `access_request` 表每条申请永久保留状态 / 审批人 / 审批时间 / 授权有效期，审批动作另落 `access_request.decided` 审计；但控制台「待我审批」只查 `pending`，审批人处理完一条它就从页面上消失了，「我发起的申请」倒是全状态可见却只限申请人本人——审批人回头想查「上个月我批过谁、批的什么、有效期到几时」没地方看。新增 `GET /api/access-requests/history`：① **范围与 inbox 同一条规则**——按环境归属而不是按「谁点的批准」，管理员见全部、其他人见自己 Own 的环境上的申请；管理员替 Owner 批掉的申请，Owner 在自己环境的历史里同样看得到；② 只含已处理（`approved` / `rejected`），按审批时间倒序，分页信封 `{items,total,page,pageSize,counts}` + `status=all|approved|rejected` 筛选（`counts` 在范围内按结果分别计数、不随筛选变，回答「切过去有几条」；`pageSize` 上限 1000、默认 20）——待办清单不需要分页，历史会一直涨；③ `AccessRequestInfo` 加 `decidedByName`（此前只有 `decidedBy` 的 id，历史清单要显示是谁批的），三个清单接口共用一条 join 查询（申请人 / 环境 / 审批人一次取齐，替换掉逐行查名字的 N+1），inbox 的「非管理员只看自己环境」顺手从取回后过滤改进 SQL（此前先 `limit 100` 再过滤，多了会漏）。控制台：「待我审批 / 我发起的申请 / 历史审批」三个页签，历史页签配描边分段「全部 / 已批准 / 已驳回」（带数量；照决策 43 的做法与页签长得不一样，且只在该页签出现——它不作用于另外两张表）+ 分页条，筛选与页码同步 URL（`?status=` / `?page=` / `?pageSize=`，本页 `useTabParam` 换成 `use-query-params`）；表列为申请人 / 环境 / 变量 / 理由 / 状态 / 审批人 / 审批时间，已批准的在状态下方补一行「有效至 …」或「永久有效」（三张表共用），待办与我发起的两张表的时间列明确标「申请时间」。**只在控制台开口子**：CLI `eat env requests` / MCP `get_access_request_status` 仍只查自己发起的申请（顺带多了 `decidedByName` 字段），审批是人的事、AI 不需要翻别人的审批历史。**记录的保留边界不变**：环境或申请人被删除时申请记录级联删除（`environment_id` / `requester_id` 都是 `onDelete: cascade`），这是既有行为，历史清单只是把还在库里的记录展示出来。CLI 命令行为未变，但 `AccessRequestInfo` 契约进了 CLI 产物，按版本号约定升 0.5.11；内置指南内容未变、版本不动 |
| 46 | 应用详情弹窗里的 Git 仓库地址旁加「GitHub」跳转链接 | **仓库地址是给 Dokploy 拉代码用的，人看到它却是想去仓库页面。** 应用的 `repoUrl` 什么写法都有——GitHub 的 scp 式 SSH（`git@github.com:o/r.git`）、`ssh://`（可带端口）、https（可带 `user:token@`）、直接贴的网页地址（带 `/tree/main` 之类子路径），也可能是自建 Git 服务或 GitLab 的地址；控制台此前只原样显示，想看仓库得自己改写地址再贴到地址栏。新增 `githubRepoWebUrl()`（`packages/shared/src/deploy.ts`）：把三种写法（带协议的 URL / scp 式 `[user@]host:path` / 不带协议的 `host/path`）拆成主机 + 路径，主机（去 `www.`、不分大小写）必须是 `github.com`，路径只认前两段 `owner/repo`（去 `.git`，丢 query / fragment / 子路径），owner / 仓库名按 GitHub 的字符集校验，回 `https://github.com/owner/repo`；其他主机一律回 `null`——各家 Git 服务的网页路由不一样（GitLab 有子组、Gitea 自定义前缀），不做通用推断，以后要支持也是按主机逐个加。**只在点击应用名打开的详情弹窗「概览」里加**：仓库地址后面跟一个 `↗ GitHub` 小链接（`whitespace-nowrap`，窄屏下不会被拆成两行），`target="_blank"` + `rel="noreferrer"` 新页签打开；清单页的 Git 列不加（那列本就是 truncate 的次要信息，点名字进弹窗一样能到）。**纯前端解析**：不动 `AppInfo` 契约、不动服务端，CLI / MCP 不涉及（AI 拿到 `repoUrl` 自己就能推）；`githubRepoWebUrl` 放在 shared 是为了可单测且以后 CLI 想用时不用搬，目前 CLI 没引用、tsup 摇掉了，构建产物逐字节一致，按版本号约定 CLI 不升、指南不动；单测放 `apps/cli/test/git-url.spec.ts`（shared 自己没有测试跑器，照 frontmatter.spec.ts 的先例）。桌面 1440 / 移动 390 双视口 Playwright 走查：SSH / HTTPS / 自建三种地址各挂一个应用开弹窗，前两者出链接且点击确实开了新页签、后者不出，无横向溢出、无控制台报错（§3.7.1） |
| 47 | `eat login` 的授权链接自带短码，登录跳转保留 query string | **控制台早就支持按 `?code=` 回填，只是没人生成这种链接。** `eat login` 打印的 `verificationUri` 此前固定是 `<publicUrl>/device`，用户点开是个空输入框，得回终端抄一遍 `AB2C-3DEF`；而 `/device` 页从一开始就拿 `params.get('code')` 当输入框默认值，这条能力一直空着。改两处：① 服务端 `deviceStart()` 把 `verificationUri` 拼成 `<publicUrl>/device?code=<userCode>`（`encodeURIComponent`，短码字符集本就 URL 安全）——放服务端而不是 CLI 侧拼，已装的旧 CLI 不用升级就能拿到带码链接；② `RequireAuth` 的登录跳转 `next` 从只带 `pathname` 改为 `pathname + search`——不然未登录用户点链接 → 登录 → 回到 `/device` 时 `?code=` 已经丢了，回填等于没做（登录页本就 `navigate(next)`，带 query 的路径直接可用；无 query 的页面 `next` 不变）。CLI 输出不动：仍打印「1. 打开链接 2. 输入代码」，短码继续显示——手动敲地址、或把链接转给别的浏览器时还用得上，`scripts/demo/record-cli.mjs` 也靠「输入代码」这行从输出里取码；CLI 代码未变、版本不升、指南不动。e2e 补断言（`verificationUri` 以 `/device?code=<短码>` 结尾且与 `userCode` 一致）；Playwright 走查：未登录点链接 → 登录页 `next=/device?code=…` → 登录后落回授权页且输入框已回填 → 点确认 → CLI 侧 poll 拿到 token；已登录直开带码链接同样回填；普通受保护页的 `next` 仍是纯路径 |
| 48 | 控制台增加「已授权的设备」页，从授权页底部的 link 入口进入 | **数据和接口早就有，缺的是页面。** 每次设备码授权都在 `api_token` 表签发一条 `kind='cli'` 记录（备注 / 授权时间 / `last_used_at` 由认证守卫每次请求刷新 / `revoked_at`），`GET /api/auth/tokens` + `DELETE /api/auth/tokens/:id` 也一直在，但控制台没有任何页面调它们——§3 写的「Token 可在控制台查看、吊销」此前没落地，`eat logout` 提示的「请在控制台吊销」无处可点。新增 `/device/authorized`「已授权的设备」页（`pages/AuthorizedDevices.tsx`）：列设备备注 / 授权时间 / 最近使用（相对时间、悬停看完整时间、没用过标「从未使用」）/ 「吊销」带二次确认，窄屏把授权时间收进第一列；页头「授权新设备」回 `/device`，空态给「去授权一台设备」。**范围三条**：① 只列 `kind='cli'` 且未吊销的——网页登录会话不算设备、不列；吊销过的记录服务端照旧返回（e2e 靠它断言 `revokedAt`），页面过滤掉；② **清单独立成页、不和授权表单铺在一起**：`/device` 授权页只在卡片下方（成功态在「继续授权其他设备」下方）加一个 link 风格的「查看已授权的设备 →」入口，路由挂在 `/device/` 下面让侧边栏「设备授权」保持高亮；③ 不加改名、不记 IP / 主机名 / CLI 版本——表里没有这些列，设备只靠授权时填的备注辨认，要更多设备信息是另一档改动。契约 `apiTokenSchema` 补 `kind`（服务端本就返回、只是没声明），它进了 CLI 产物，按版本号约定 CLI 升 0.5.12；服务端不动、指南不动。e2e 补断言（清单里设备是 `cli`、同时能看到 `web` 会话、吊销后 `revokedAt` 非空）；Playwright 走查：授权一台设备 → 点入口 → 清单里出现且「从未使用」→ 用该 Token 调一次接口后显示「刚刚」→ 吊销 → Token 立即 401、清单回到空态；桌面 / 移动双视口截图 |
| 49 | 时间只在展示端按本地时区格式化，三端共用 shared 的 `formatDateTime` / `formatDate` | **给容器加了 `TZ=Asia/Shanghai` 后页面仍显示 UTC，因为页面上的时间根本不是服务端格式化的。** 链路是：服务端所有时间列 `toISOString()` 传绝对时刻（永远是 `…Z`、不受 TZ 影响，这一步是对的）→ 浏览器自己格式化。控制台此前有**两个同名 `formatDateTime`**：`lib/format.ts` 的用 `toLocaleString` 按浏览器时区换算（环境 / Skill / 设备 3 个清单页在用），`lib/utils.ts` 的只是 `iso.slice(0, 16).replace('T', ' ')`——把 UTC 字符串裁一下直接显示、注释里写的「本地化」是假的（应用 / 权限申请 / 求助清单与详情 / 环境详情 / Skill 详情 / 数据库分配详情 7 个页面在用），同一个控制台里清单页是北京时间、点进详情又变回 UTC。CLI 也有三处同样裁字符串（`eat ask` 对话时间、`eat env requests`、`eat app` 部署 / 构建清单），服务端给设备取默认名 `CLI（YYYY-MM-DD 授权）` 用 `toISOString().slice(0, 10)` 是 UTC 日期、北京时间凌晨 0–8 点授权会记成前一天。**改法**：新增 `packages/shared/src/time.ts`——`formatDateTime()` 出 `YYYY-MM-DD HH:mm`、`formatDate()` 出 `YYYY-MM-DD`，都按**所在进程的本地时区**换算（浏览器 = 用户时区、CLI = 终端所在机器、服务端 = 容器 TZ），接受 Date 或 ISO 字符串、空值与无法解析回 `—`；刻意不用 `toLocaleString`（各浏览器 / Node 版本输出略有差异，固定格式更好扫）。**控制台只留一个入口**：`lib/format.ts`（`formatDateTime` 转发 shared + 本地的 `formatRelativeTime`），`lib/utils.ts` 那个删掉、7 个页面改 import；3 个清单页的 title 提示从 `toLocaleString` 的「2026/9/14 09:54:00」变成「2026-09-14 09:54」。CLI 三处与服务端默认名改用同一份。服务端序列化不动（仍 `toISOString()`），数据库与接口无变化；单测 `apps/cli/test/time.spec.ts` 切 `process.env.TZ` 断言同一时刻在上海 / UTC / 纽约的输出（含跨日）。CLI 升 0.5.13、指南不动。**教训**：`TZ` 只影响服务端进程里的本地时间输出（日志时间戳、`formatDate(new Date())`），不可能影响浏览器；以后页面或 CLI 要显示时间一律走这两个函数，别再 slice ISO 字符串 |
| 50 | MCP 配置分两层权限：可见性 + 订阅审批 | **MCP 配置此前只有「私有 / 团队可见」一个开关，且团队可见 = 谁都能订上就同步走**，配置里指向的内部服务、内网地址、以及「这个团队有哪些内部系统」本身就是信息，不该人人自取。改为两层：① **可见性**——`team` 全员可见（看得到名称与说明、可自助申请），`private` 不公开（只有 Owner / 管理员看得到，成员只能被动接受分配）；② **订阅审批**——成员订阅 = 提一条申请（可带理由），由**配置 Owner 或管理员**批准后才进 `eat sync` 范围，与环境变量的读取授权申请同一套心智（审批人口径也照抄「环境 Owner 或管理员」，成员自建的配置由自己审批，管理员始终可越权审批）。落地要点：**审批状态存在订阅行上而不是另开申请表**（`mcp_subscription` 加 `status` / `reason` / `decided_by` / `decided_at`，迁移 `0013_mcp_subscription_approval`），「谁订阅了」与「批没批」本就是同一条关系，拆两张表只会产生对不齐的两份真相；**存量订阅行默认 `approved`**（加列时的 DEFAULT 就地填好），线上已有的订阅不因为这次改动断掉。三个踩点：**已 approved 的行不能被重复点订阅打回 pending**（成员对着已分配的配置再点一次订阅就会把自己的权限点没了）；**canSee 必须把「已订阅 / 申请中」算进去**，否则管理员分配下去的不公开配置会「同步得到却在清单里查无此物」；**审批清单只认自助申请**（`source=manual` 且不是自己批自己），否则管理员分配出去的订阅、Owner 自己的订阅都会混进审批列表。审批入口放在 MCP 配置页页头的「订阅申请」弹窗（带待审批圆点，默认只看待审批、可切「全部」），不并进权限申请页——那边的历史审批是服务端分页的，混进第二类资源要各开一套分页接口与类型筛选，改动明显更大；权限申请页只加一句指引链接。订阅者明细与「主动分配」收在每行的「订阅者」弹窗里（模板派生的订阅在这里移不掉，只能去改模板）。CLI / MCP 不加命令（AI 不需要替用户要或批 MCP 的分发权），但契约进了 CLI 产物，按约定升 0.5.14；内置指南未变。 |
| 51 | MCP 配置改为经平台网关分发：每人一条专属接入地址 | **此前成员订阅一个 MCP 配置，拿到的就是真实配置本身**——`eat sync` 把 `${env:...}` 解析成明文上游凭证写进 `~/.eat/mcp.generated.json`。这有三个问题：凭证是团队共享的一份、落在每个订阅者磁盘上；**退订只删订阅行，对已经落盘的凭证毫无作用**，想真正收回只能轮换上游凭证并通知所有人；平台看不到任何调用。改为平台代理：`http` 配置默认经网关分发，成员拿到 `<平台>/mcp/<slug>/<token>`，**路径里的 token 即鉴权**，上游地址与 header 不下发。关键设计：① **凭证按配置 Owner 的授权解析**（`readValuesForGateway`），网关模式下「订阅被批准」就是授权，成员不需要也不该有对应变量的读取权限——否则他 `eat env pull` 就能拿走凭证，网关白做；按配置粒度缓存、记 `mcp_gateway.credentials_resolved`，不按次写 `secret.read`（会把审计表冲掉）。② **失效靠两条机制叠加而非逐处吊销**：授权至少能从六条路径消失（退订 / 驳回 / 移除订阅者 / 管理员改模板内容 / 用户改选模板 / 配置删除或用户禁用），逐个加吊销代码早晚漏一条，漏的那条就是永久有效的后门 URL——所以每次请求**实时复算**授权（不通过就地标记吊销），且每次签发都是**新建 + 吊销该 (user, config) 上所有旧 token**；用户要的「取消后重新获取即换新地址、旧地址永久作废」由此自动成立，连模板那两条路径都不用碰。③ **token 必须可逆存储**（信封加密，同 KEK）：成员每次 sync / 每次开控制台都要拿回同一条地址，只存哈希会变成「每次同步换地址」，已配好的客户端全废、多机也用不了；查询仍走 `token_hash` 唯一索引。④ **签发点判的是「是否跨过有效这条线」**（`syncGatewayAccess(userId, config, wasEffective)`），不是「有没有点过订阅」——已生效的人重复点一次订阅不该把他正在用的地址换掉（模板派生的订阅尤其容易触发）。⑤ **SSRF 必须挡**：`upsert` 没有管理员限制，任何成员都能建配置，不挡就等于把平台变成成员可驱动的 SSRF 跳板（云 metadata 169.254.169.254、平台自己的内网服务都在射程内）；解析后校验 IP、拒私网/回环/保留段、不跟随重定向（3xx 回传等于交出上游地址），内网上游要显式开 `EAT_MCP_GATEWAY_ALLOW_PRIVATE=1`。⑥ **响应头白名单**：`www-authenticate` 按 MCP 鉴权规范会带上游 resource metadata 地址，原样透传就把要藏的东西漏回去了；`set-cookie`/`location` 同理。⑦ **旧版 HTTP+SSE 传输明确拒绝而不是半代理**：它的 `endpoint` 事件里是上游地址，要正确代理得重写并再开一条路由，本期选择报错——泄漏上游比不支持旧传输严重得多。**stdio 强制不走网关**：本地进程没有可代理的端点，要藏凭证得由平台托管执行，是另一个量级的工程，本期不做（清单与编辑页会说清「这类配置的密钥会落到你本地」）。调用记录单独建表 `mcp_gateway_call`（不写 `audit_log`：那是安全事件，这是每次工具调用一行的流量数据，量级差几个数量级）、**只记 method 与工具名不记 arguments**、保留期默认 90 天由模块内每日清扫（server 里没有任何调度器，pg-boss 在选型里但从未引入，为一张表引队列不成比例）。**破坏性变更**：存量 `http` 配置随 `DEFAULT true` 一次性迁到网关分发，订阅者必须重跑 `eat sync` 并更新客户端里的 MCP 配置（地址变了）；`McpConfigInfo` 的 `url`/`headers`/`env` 对非 Owner 不再下发。顺带修掉两个既有问题：控制台编辑弹窗**根本没有 headers 编辑器**（`toPayload` 写死 `headers: {}`，点一次「编辑」就把配置的 header 清空——网关凭证正放在 header 里，不补上这个功能就没法用）；Fastify 对 `content-type: application/json` 且空 body 的请求直接回 400，而 **MCP 客户端结束会话发的正是 `DELETE` + 空 body**，请求根本进不了网关（改为宽容解析器，只放宽「空 body」这一种，非空仍按 JSON 解析、失败照旧 400）。迁移 `0014_mcp_gateway`，CLI 升 0.5.15、指南升 15。 |
...
| 56 | `eat sync` 的落点由配置记住，并对「静默回落到默认目录」设拦截 | **问题不是缺一个自定义目录参数（`--dir` 决策 14 就有了），而是它只对当下这一次有效。** 更新提示（决策 26）里写的是裸 `eat sync`，而裸 `eat sync` 此前恒等于 `--global`，于是用 `--dir /srv/agent/skills` 装好的环境，一收到「团队 Skill 有变更」就会被同步到 `~/.agents/skills`，原落点从此不再更新；更糟的是 `markSkillsSynced()` 会把服务端指纹记成本地基线，**提示随之消失**——整条链路静默且自我掩盖，没有任何一处会再出声。旁证：`localSkillVersion()` 此前硬编码只扫 `~/.agents/skills`，`--dir` 用户在还没有基线时必然被判成「需要 sync」，等于现在就有一条自动把人引向错误落点的路（一并修掉，目录改由调用方按配置解析后传入）。**改法是让 CLI 自己记得住落点**：① 落点解析改为 `命令行 > 环境变量 EAT_SYNC_SCOPE/EAT_SYNC_DIR > 项目配置 ./.eat/config.json > 用户配置 ~/.eat/config.json > 内置默认`（与 `resolveServerUrl` 的惯例一致），作用域是**整体覆盖不是按字段合并**——命令行给了 `--project`，配置里的 scope 与 dir 一并让位，否则配置的 dir 会和命令行的 project 撞上「三者互斥」那条校验、报一个没人看得懂的错；② **显式指定过就自动记住**（`--dir` / `--global` 写用户配置，`--project` 写**项目内**配置，`--no-save` 可跳过）——只给一条 `eat config set` 让人手动敲是不够的，安装往往由 AI 代跑，它装完就走，没人会补那一条，坑原样留着；`--project` 之所以不能写进用户配置，是它按 cwd 解析，写全局会让换个仓库裸跑 sync 也变成项目级，等于造出新的跑偏路径（这是「项目配置优先于用户配置」这层查找顺序存在的唯一理由）；③ **决议先于动作打印**（作用域 / 来源 / 落地目录 / `.claude` 去向），中途失败也看得见落点，而不是只在成功的末尾打一行；④ 更新提示本身带上落点（`团队 Skill 有变更 —— 更新: eat sync（落点：…）`），Agent 照着裸跑之前就能看出会装到哪；⑤ 新增 `--dry-run` 只打印落点与将要新增/更新/移除的内容、**不写任何文件**——尤其要跳过 `markSkillsSynced()` 与 `mcp.generated.json` 的写入，否则一次预演就把更新基线冲掉、真正的提示反而消失；⑥ 新增 `eat config list/get/set/unset`（只开 `sync.scope` / `sync.dir` 两个键，不开通用配置面），`sync.dir` **一律存绝对路径**——存相对路径换个 cwd 跑就落到别处，比没有配置更难排查；切到非 dir 作用域时丢掉 dir，留着它以后切回来会悄悄复活一个早就不对的旧目录。**落点漂移检测只在回落到内置默认时触发**：上次装在别处、这次却没有任何东西指定落点（配置丢了 / 换了机器 / 被手删），就以退出码 1 中止并给出三条出路（装回原处 / `--yes` 确认改用默认 / `--dry-run` 先看）。这条边界是踩出来的：最初把「配置给出的落点与上次不同」也算漂移，结果在项目目录与自定义目录之间来回同步这种正常用法下次次误报——**次次误报的拦截等于没有拦截**，用的人只会固定加上 `--yes`；配置是可见、可查、刻意写下的东西，不该拦。同理**预演不拦**，它正是中止信息里给的排查办法，拦掉等于堵死出路。`eat config set/unset` 改动同步配置时清掉落点基线（改配置本身就是「我知道落点要变」的授权，不该在下一次 sync 时再被拦一道）；环境变量**不落盘**（本进程的一次性设定，持久化会让人意外）。为什么不采用「`eat sync` 默认 dry-run + `-y` 执行」：Agent 没有上下文知道默认落点是错的（安装可能是上一个会话、上一个人做的），看到一个正常的默认路径照样会 `-y` 确认下去，**拦一道不等于拦对**；而一旦 CLI 自己知道正确答案，裸跑本来就落对，那道确认也就不必要了——且默认 dry-run 会让决策 26 那句「Skill 更新执行 `eat sync`」变成空转。同步改两处 AI 入口：`/install/AGENT.md` 第 3 步改成「先确认你的 skill 放在哪再选命令」并写明 `--dir` 不创建 `.agents` / `.claude`、落点会被记住；内置指南速查表补 `--dir` / `--dry-run` / `eat config` 与「照着提示裸跑、不要自己加作用域参数」（指南升 18）。CLI 升 0.5.18。 |
| 57 | 数据库申请放开给 AI：MCP 补 `list_db_instances` / `request_db` / `list_db_assignments` | **CLI 早就有 `eat db instances / request / list` 三条命令，MCP 一个工具都没有，而内置指南里写的是「需要新账号时，引导用户在控制台『数据库』页申请」** —— 于是走 HTTP MCP 接入的云端 AI（没有 shell、没有 CLI）想给自己建个库放跑批结果，唯一的出路是让人替它去点一遍界面；有 CLI 的客户端也因为指南这句话而不去用那三条命令。这不是刻意的门禁：申请本身就是一次「填用途 → 等管理员批准 → 批准后平台真实建库建号并下发凭证环境」的常规动作，和 `request_access` 同一个形状，而 `request_access` 一直是开放给 AI 的。真正的门禁在审批那一步，它没有被削弱——`request_db` 建出来的仍是 `pending`，仍由管理员在控制台批。三个实现要点：**① 实例可以用名字引用**——AI 手上多半是刚从 `list_db_instances` 读到的名字而不是 uuid，CLI 的 `--instance` 本来就认名字，MCP 不认会导致一轮无谓的往返；判定抽成 `packages/shared/src/db.ts` 的 `resolveDbInstance()` 供 CLI 命令、CLI stdio MCP、服务端 HTTP MCP 三处共用，**不能各写一份**，否则「这个名字算不算数」早晚在某条路径上给出不同答案。**② 实例名找不到时把候选清单一起回过去**（`DB_INSTANCE_NOT_FOUND` + `instances[]`），名字打错是这个工具最常见的失败，省掉「报错 → 再列一次 → 再试」这一轮。**③ 指南要写清楚什么时候才该申请**：只读地取别人的业务库走环境变量那条路，**需要自己写数据（建表、存跑批结果）时才申请库**——否则 AI 会把「要连数据库」一律理解成「要申请一个库」，平台上堆满一次性的空库。服务端 `McpToolsService` 直接调 `DbsService`（`DbsModule` 补 `exports`），权限与审计仍在服务层那一份实现里，MCP 这层只做参数校验与结果整形。CLI 升 0.5.19、指南升 19（数据库账号一节重写 + 速查表那行从 `eat db list` 扩成三条命令）。 |
| 58 | MCP 网关按「长耗时调用是常态」重做超时与断开判定：上限默认 3 分钟且可配，超时给 504 | **现象是「经网关代理的 tool 调了几十秒，客户端什么都没收到」**，而且平台的调用记录里连这次调用都不存在。两个原因叠在一起：① 网关等上游响应头的上限是写死的 30 秒——大多数 MCP 服务是**等结果出来才一次性发响应头**，所以这 30 秒管的其实是整次 `tools/call` 的耗时，而跑几十秒到几分钟正是长任务型工具的常态；② 更要命的是超时之后的处理：代码拿 `req.raw.destroyed` 当「客户端已经断开」的判据，而 **Node 16 起 `IncomingMessage` 的 `close` 在请求体读完时就触发并把流标记成 destroyed**，跟「客户端断开」根本是两回事——POST 进到 handler 时它早就是 true 了。于是每一次上游超时都被当成「客户端自己走了」，走 `reply.raw.end()` 发出一个**没有 body 的 200/201**：客户端拿不到任何 JSON-RPC 响应（表现就是「调用了但什么都没回来」），平台侧也不留痕，两头都查不出问题在哪。**静默失败比报错难查得多**，这条和决策 52「填错地方就静默失效」是同一类。现在：上限提到默认 3 分钟并由 `EAT_MCP_GATEWAY_UPSTREAM_TIMEOUT_MS` 配置（值写歪时回落默认——`NaN` 交给 `setTimeout` 等于 0 毫秒，那就成了「一配就立刻超时」），原则是**网关不该是先放弃的那一个**，等多久由客户端自己的超时决定；只管到拿到响应头为止，之后的流式传输不设限（SSE 本来就是长连接）。断开判定改看响应侧——`reply.raw` 的 `close` 且 `writableFinished` 为假才是真断开；超时另给 `504 MCP_GATEWAY_UPSTREAM_TIMEOUT` 并落调用记录。顺带修掉两个同源问题：原来那个 `req.raw.on('close')` 注册时事件早就发过了，**「客户端断开就断开上游」这件事其实从来没生效过**（客户端放弃后上游还会跑完），改到响应侧后才真正生效；转发**从 `fetch` 换成 `node:http`**——Node 内置 fetch 底下的 undici 自带 `headersTimeout` / `bodyTimeout` 两个 5 分钟硬上限，既不能按请求配也关不掉，意味着「上限配到 5 分钟以上等于没配」（被它们悄悄盖住，参数写了不生效比没有参数更糟）和「闲置超过 5 分钟的 SSE 通知流被悄悄掐掉」（这条跟上限配多少无关），`node:http` 默认无超时，配置项才是唯一说了算的那个（响应体改用 `stream.pipeline` 转发，背压与收尾一并交给它）。只动服务端，不涉及 CLI / MCP 工具 / 指南，版本号不动。 |
| 59 | 平台地址由 CLI 记住，且跳转的地址不再把访问令牌丢在路上 | **两个症状，同一个根：平台地址此前只存在凭证文件里，而「地址」和「凭证」根本不是一回事。** ① `eat logout`（或凭证过期后被删、被手工清掉）会把 `serverUrl` 一起带走，下一次裸跑 `eat login` 于是回落到 `http://localhost:3000`——用户以为在登录团队平台，实际在连本机，运气好报一句连不上，运气不好本机正好有服务在 3000。`eat self-update` 同样按这个地址去平台取产物。② 更隐蔽的是跳转：**`fetch` 规范在跨源跳转时会丢掉 `Authorization` 头，而「只换 scheme」就算跨源**。平台前面挂一个把 http 跳到 https 的反代（Traefik / nginx 的标准配置）之后，所有用 `http://` 地址登录过的存量客户端，请求都会在跳转那一跳被摘掉令牌，服务端只看到一个没带令牌的请求，于是回 `缺少访问令牌，请先登录`——**这句话把人引向重新登录，而重新登录修不好**：设备码接口是 @Public 的，跳转过去照样成功，`acceptToken` 存下的仍是那个会跳转的旧地址，登录显示成功、下一条命令继续报同样的错，只有先 `eat logout`（把旧地址丢掉）再 `eat login --server <最终地址>` 才能好转，用户看到的就是「必须先退出登录再重新登录一次」。跳转码是 301/302 时还要更早一步就崩：fetch 会把 POST 降级成 GET，`eat login` 直接报 `Cannot GET /api/auth/device/start`。**改法一：地址单独记一份**——写进用户配置 `~/.eat/config.json` 的 `server`（决策 56 已经有这个文件），解析顺序 `--server > EAT_SERVER > 已保存凭证 > 记住的地址 > 内置默认`；登录成功时记，`eat logout` **先把凭证里的地址搬进配置再删凭证**（存量用户第一次用新版退出登录即完成迁移），并明确打印「平台地址仍记着 X」；`eat config get/set/unset server` 可查改（只写用户配置——一个人登录哪台平台与他此刻在哪个仓库无关）。**改法二：跳转自己处理**——所有请求改 `redirect: 'manual'`，按目标分三类：同源照跟；**同主机的 `http → https` 跟随并把修正后的地址写回凭证与配置**（令牌仍然只发给同一台主机，且从明文升级到 TLS，是严格变好；存量客户端装上新版后第一条命令就自愈，提示一次即可）；**换了主机一律停下报 `PLATFORM_REDIRECT`**，把最终地址和该执行的命令一起给出来——绝不把令牌转发给另一台主机（http 那一跳可能被劫持，跟过去就是把长期 Token 送人）。跟随时方法与 body 原样重发，不学 fetch 把 301/302 的 POST 降级成 GET：这里跟的只是同主机的基础设施跳转。登录落盘的地址取 `api.resolvedUrl`（跳转修正后的那个）而不是入参，否则「登录成功但每条命令都没令牌」的坑原样留着。**改法三：把无解的提示变成有解的**——`Api.fromSaved()` 把「有凭证文件但没有 token」当成未登录（带着它发请求只会换来同一句「缺少访问令牌」）；带着令牌还被告知缺少令牌时，CLI 在服务端那句话后面补一句「本次请求确实带了访问令牌，是平台没收到，请检查反向代理是否转发 Authorization 头」——前置代理吃掉请求头是另一条会走到同样症状的路，不补这句，用户只会反复重新登录。服务端一行未改。CLI 升 0.5.20；指南未变（对 AI 的用法没有变化）。 |
| 60 | 网关转发失败要记下**技术原因**，而不是只留回给调用方的那句安慰话 | 回给调用方的话是刻意模糊的（照决策 33，不泄漏上游）：一句「这个服务暂时连不上，请稍后重试或联系配置负责人」。问题是这句话**同时也是留在调用记录里的全部内容**——而一次 502 背后可能是 DNS 解析不了、连接被中途重置、TLS 握手失败、上游直接拒连，这些差别恰恰决定了该去查谁（是配置写错了、是上游挂了、还是平台这台机器出不去网）。`err` 对象在 catch 里被整个丢掉，于是每一次网关故障的排查都从猜开始，而这类问题**必然发生在线上、必然不可本地复现**，猜的成本最高。现在：`describeCause(err)` 取 `code`/`name` + `message`（截断到 500 字）作为 `detail`，**只进 `mcp_gateway_call.error` 与服务端日志，不进回给调用方的 body**；超时那条把「等了多少毫秒、该调哪个环境变量」一起写进去，3xx 那条写明「上游回了 30x 跳转，平台不跟随」。同时补一条 `Logger.warn`（slug / method / 工具名 / 耗时 / 原因，上游地址与凭证一个字不打）——Dokploy 这类部署里，人第一眼看的是容器日志而不是控制台。控制台的调用记录把失败原因**从 `title` 提示里摊到行上**：藏在悬停提示里等于没有，排查时要看的就是这一句。e2e 补一条「上游拒连」的用例，断言回给调用方的 body 里不含上游地址、而记录里含 `ECONNREFUSED`。只动服务端与控制台，协议与 CLI 不受影响，版本号不动。 |
...
| 67 | 更新提示不再按版本去重：更新之前每条命令都提示 | **团队期望所有人始终用最新版，而决策 26 的「同一版本只提示一次」让提示几乎必然被错过。** 去重标记记在本机共享的 `~/.eat/state.json` 里，所以提示是被**第一个跑 eat 的进程**「用掉」的：人在终端里看过一眼（或某个 Agent 会话收到后没处理、上下文被压缩、会话结束），之后本机所有 Agent 会话都再也收不到——实测维护者本机停在 0.5.18、平台已是 0.5.24，Skill 指纹也对不上，而跑任何 eat 命令都一片安静。当初去重是怕一个任务里连跑十几条命令把同一段话刷十几遍、让 Agent 纠结要不要中断手上的活；但这点上下文占用（每条命令 4 行 stderr）远比「落后六个版本没人知道」便宜，而「要不要中断」交给文案与指南解决。现在：① **CLI 每条命令收尾都提示**，直到 `eat self-update` / `eat sync` 让条件消失；进程内闸门保留（一条命令只打一次）。② **MCP（stdio）仍是一个 server 进程附一次**：server 通常与客户端会话同生共死，每个新会话都能看到一次，再往每次工具返回里塞同一段话不增加信息。③ 文案首行改为「不影响本次命令结果；更新之前每条 eat 命令都会附这段提示」，告诉 Agent 反复出现不是新情况；内置指南从「先做完或在两个任务之间顺手执行」改为「当前任务一做完就执行，别拖」。④ `eat self-update` 覆盖产物后，正在跑的仍是旧代码，不做处理的话这条命令收尾会紧跟着提示「请 eat self-update」——改为记下本进程已装的版本（`installedCliVersion`），而不是像以前那样写去重标记。⑤ 老 CLI 留在 state.json 里的 `notified*` 字段一律忽略，不做清理。关闭开关 `EAT_NO_UPDATE_NOTIFIER=1` 不变。CLI 升 0.5.25、指南升 24 |
