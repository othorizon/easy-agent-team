...
| **捆绑模式** | 管理员开启后，该 Skill 对所有成员恒为已订阅、不可退订，`eat sync` 总会同步；要求团队可见（§10 决策 37）。管理员可对个别成员「解除捆绑」，由其自行决定是否订阅（§10 决策 45） |
...
- 用户可在模板基础上**排除**个别不想要的条目；**捆绑的条目不可排除**（退订接口对成员直接返回 `SKILL_BUNDLED`），管理员自己不受捆绑约束、仍按自己的订阅记录算；管理员可在订阅者名单里为个别成员**解除捆绑**（`skill_bundle_exemption`），该成员对这个 Skill 回到普通的自行订阅 / 退订，随时可恢复；整个 Skill 取消捆绑时豁免整批清掉（决策 45）；
- **订阅人数**对所有人可见（口径 = 有效同步人数，只算启用中的用户）；**订阅者名单与代订阅仅管理员**：管理员可在 Skill 详情页看到每个订阅者的来源（手动 / 模板 / 经验沉淀 / 捆绑），并直接为某人订阅或取消订阅；捆绑 Skill 上还能对某个成员解除 / 恢复捆绑，没自己订的被解除者单列在名单下方（决策 45）。
...
| Skill（捆绑） | 元数据+内容可见 | 管理员开启捆绑，全员强制订阅、成员不可退订；管理员可对个别成员解除捆绑（决策 45） | Admin |
...
**skill_bundle_exemption**：user_id, skill_id, created_by, created_at（捆绑豁免：管理员为个别成员解除捆绑，user+skill 唯一，决策 45）
...
| 45 | 捆绑 Skill 可对个别成员「解除捆绑」 | **捆绑是「全员必装」，但总有个别成员确实用不上（岗位不同、与本地配置冲突），此前唯一的出路是整个 Skill 取消捆绑——为一个人放弃对其他人的强制，不成比例。** 管理员现在可以在捆绑 Skill 的订阅者名单里对某个成员「解除捆绑」：这个 Skill 对该成员不再强制订阅，回到与普通 Skill 一样的自行订阅 / 退订；随时可「恢复捆绑」。① **落库**：新表 `skill_bundle_exemption`（user_id / skill_id / created_by / created_at，user+skill 唯一，迁移 `0013_skill_bundle_exemption`）。**不复用 `skill_subscription.excluded`**：豁免是管理员的决定，要跨成员自己的订阅 / 退订持久存在，而 `excluded` 会被 subscribe / unsubscribe 反复翻转、退订时整行删掉。② **语义收敛到一个集合**：`bindingSkillIds(user)` = 全部捆绑 − 已为其解除的（管理员恒为空集），`subscriptionLocked`、`relation=bundled`、退订被拒（`SKILL_BUNDLED`）三处判断都以它为准，不再各自看 `skill.bundled`；有效同步集合改为「订阅 ∪（模板 − 排除）∪ 对其生效的捆绑」，人数与名单共用的 `effectiveSubscribers()` 同步减去被解除的成员。解除**即刻生效**：没自己订过的，下次 `eat sync` 就移除本地副本；自己订过的（捆绑前订的、或解除后又订的）照旧保留，名单里标「已解除捆绑」、可退可移。③ **接口（仅管理员）**：`GET/POST /api/skills/:slug/bundle-exemptions`、`DELETE …/bundle-exemptions/:userId`；解除拒绝非捆绑 Skill、管理员（本就不受约束）与禁用用户，解除与恢复都幂等；代订阅 / 代取消对已解除的成员放行（其他成员仍回 `SKILL_BUNDLED`）；`SkillSubscriber` 加 `bundleExempt`。**取消捆绑时整批清掉豁免**：将来再捆绑是一次新的「全员必装」决定，不让旧豁免悄悄复活、让某些人莫名其妙地没被装上。④ **UI**：订阅者名单里被捆绑的行把原本空着的操作列换成「解除捆绑」（非破坏性确认）；没自己订的被解除者不在名单里了，所以名单下方单列「已解除捆绑（N）」——姓名 / 邮箱 / 谁何时解除 / 是否已自行订阅 + 「恢复捆绑」；「添加订阅者」在捆绑 Skill 上也开放（能选的只剩未自行订阅的管理员与已解除的成员）。成员侧不需要新 UI：`subscriptionLocked=false` 后详情页 / 清单的订阅按钮自然解锁。⑤ **CLI 行首 `◆` 改按 `subscriptionLocked` 判**而不是 `bundled`：管理员与被解除的成员对它照常可订可退，标 ◆ 会让图例里的「不可退订」说谎，标签列的「捆绑」字样仍按 Skill 本身；内置指南补一句「确实不需要的话请管理员为你解除捆绑」。顺带给 `getTargetUser()` 加 `isFullId()` 挡板，代取消订阅路径上乱写的 userId 从 500 变 404。⑥ 验证：server e2e 190 → 197（权限 / 非捆绑与管理员被拒 / 解除后清单、详情、sync-bundle、人数、名单、豁免清单 / 自行订退 + relation=subscribed / 代订阅放行 + 其他成员仍拒 / 幂等 / 恢复 / 取消捆绑清豁免），CLI 单测补行首标记的解锁分支，控制台桌面 1440 / 移动 390 双视口 Playwright 走查（解除 → 成员按钮解锁并自行订阅 → 名单标「已解除捆绑」→ 恢复捆绑，无横向溢出、无控制台报错）。CLI 升 0.5.11、指南升 14（§3.2.1、§3.2.2、§4、§5.2） |
