...
6. 支持回收：禁用账号（可恢复）；删除分配为**仅记录级软删除**（§10 决策 13）——删除平台记录与凭证环境，实例上的库与账号**不做物理删除**，控制台二次确认时明确提示「只能到数据库实例上手动删除」。
