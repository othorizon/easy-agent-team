...
4. **分配结果自动生成为一个环境（或环境变量组）**：`DB_HOST / DB_PORT / DB_NAME / DB_USER / DB_PASSWORD`，默认仅授权给申请人——数据库账号的下发、拉取、审计**完全复用环境变量体系**；
...
- 每个项目可配置检查项，MVP 内置两类：
  - **密钥泄漏扫描**（三层）：
    ...
    2. **平台密钥指纹匹配**（独有能力）：对平台内所有环境变量值取不可逆指纹（SHA-256），扫描代码中的高熵字符串同法比对——命中即证明真实下发的密钥被硬编码进了代码；指纹单向，规则库本身不泄密（见 §8.6）；
    3. 误提交检测：仓库中不允许出现含值的 `.env` 文件；
  - **构建可通过**：按项目配置的命令依次执行（典型 install → lint → build → test），任一非零退出即拦截；命令与超时可配。
- 检查执行方式（按接入成本递进）：
  1. **平台侧 runner**：平台拉代码在隔离容器里跑检查（MVP 推荐；无 Docker 的环境降级为同接口的子进程模式）；
  2. **CI 回调模式**：项目已有 CI 的，由 CI 完成检查后回调平台登记结果，平台只做门禁判断；
- 检查结果落 `precheck_result`（检查项/结论/完整报告），部署记录关联报告；**失败报告面向 AI 可读**——AI 经 MCP 拿到原因自行修复后重试部署；
- 暂不内置（有真实需求再加）：依赖漏洞审计、大文件误提交、Dockerfile 规范检查。
