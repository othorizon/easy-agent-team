...
stateDiagram-v2
    ...
    resolved --> open : 求助者追问（决策 65）
    resolved --> answered : 被求助者补充（决策 65）
...
- 支持**多轮对话**：请求下是一串消息，双方都可追加；**已解决的求助收到新回复会自动重新打开**（决策 65）——求助者追问回到 `open`、被求助者补充回到 `answered`，重新出现在「待回复」里并照常通知对方；只有 `closed` 不能再回复；
