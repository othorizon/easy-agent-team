...
| 命令 | 功能 |
|---|---|
| `eat login [--no-wait] [--status] [--timeout <秒>]` / `eat logout` / `eat whoami` | 设备码登录（默认阻塞等待；`--no-wait` 发起后立即返回、`--status` 查一次并领取凭证，供 AI 自动执行，决策 54）、登出、查看当前身份 |
...
| `eat config list \| get \| set \| unset` | 查改 `eat sync` 的落点（`sync.scope` / `sync.dir`），决定裸跑 `eat sync` 装到哪 |
