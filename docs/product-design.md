...
- **长耗时调用不被网关掐断**（§10 决策 58）：`tools/call` 跑几十秒到几分钟是常态，网关等上游**响应头**的上限默认 10 分钟（`EAT_MCP_GATEWAY_UPSTREAM_TIMEOUT_MS` 可配），拿到响应头之后的流式传输不设限——该由客户端决定「等多久算太久」，代理不该替它决定。超时给明确的 `504 MCP_GATEWAY_UPSTREAM_TIMEOUT` 并落一条调用记录；客户端自己断开则就地释放上游连接、不记为故障。
...
| 58 | MCP 网关按「长耗时调用是常态」重做超时与断开判定：上限默认 10 分钟且可配，超时给 504 | **现象是「经网关代理的 tool 调了几十秒，客户端什么都没收到」**，而且平台的调用记录里连这次调用都不存在。两个原因叠在一起：① 网关等上游响应头的上限是写死的 30 秒——大多数 MCP 服务是**等结果出来才一次性发响应头**，所以这 30 秒管的其实是整次 `tools/call` 的耗时，而跑几十秒到几分钟正是长任务型工具的常态；② 更要命的是超时之后的处理：代码拿 `req.raw.destroyed` 当「客户端已经断开」的判据，而 **Node 16 起 `IncomingMessage` 的 `close` 在请求体读完时就触发并把流标记成 destroyed**，跟「客户端断开」根本是两回事——POST 进到 handler 时它早就是 true 了。于是每一次上游超时都被当成「客户端自己走了」，走 `reply.raw.end()` 发出一个**没有 body 的 200/201**：客户端拿不到任何 JSON-RPC 响应（表现就是「调用了但什么都没回来」），平台侧也不留痕，两头都查不出问题在哪。**静默失败比报错难查得多**，这条和决策 52「填错地方就静默失效」是同一类。现在：上限提到默认 10 分钟并由 `EAT_MCP_GATEWAY_UPSTREAM_TIMEOUT_MS` 配置（值写歪时回落默认——`NaN` 交给 `setTimeout` 等于 0 毫秒，那就成了「一配就立刻超时」），原则是**网关不该是先放弃的那一个**，等多久由客户端自己的超时决定；只管到拿到响应头为止，之后的流式传输不设限（SSE 本来就是长连接）。断开判定改看响应侧——`reply.raw` 的 `close` 且 `writableFinished` 为假才是真断开；超时另给 `504 MCP_GATEWAY_UPSTREAM_TIMEOUT` 并落调用记录。顺带修掉两个同源问题：原来那个 `req.raw.on('close')` 注册时事件早就发过了，**「客户端断开就断开上游」这件事其实从来没生效过**（客户端放弃后上游还会跑完），改到响应侧后才真正生效；转发**从 `fetch` 换成 `node:http`**——Node 内置 fetch 底下的 undici 自带 `headersTimeout` / `bodyTimeout` 两个 5 分钟硬上限，既不能按请求配也关不掉，意味着「超过 5 分钟的调用必挂」和「闲置超过 5 分钟的 SSE 通知流被悄悄掐掉」，外面还配着一个看起来管用、实际被它们盖住的超时参数，`node:http` 默认无超时，配置项才是唯一说了算的那个（响应体改用 `stream.pipeline` 转发，背压与收尾一并交给它）。只动服务端，不涉及 CLI / MCP 工具 / 指南，版本号不动。 |
