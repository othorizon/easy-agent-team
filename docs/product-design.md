...
| 35 | 开源许可证定为 Apache License 2.0 | **项目以 Apache License 2.0 开源，版权署名 `Copyright 2026 othorizon`。** **为什么不是 MIT**：MIT 在 npm 库 / CLI 工具赛道是压倒性主流，但本项目是带服务端、数据库账号分配、密钥下发与部署编排的平台，形态落在「企业自部署基础设施」一档——这一档的惯例是 Apache 2.0（Kubernetes、Prometheus、etcd、CNCF 毕业项目硬性要求，本项目所接的 Dokploy 亦然），因为它比 MIT 多两样东西：§3 的**显式专利授权**（MIT 只字未提专利）与**专利反诉终止条款**。代价是分发时多两项义务（附许可证副本、标注改动过的文件），对本项目可忽略。落地范围**只有 `LICENSE` 文件与各 `package.json` 的 `license` 字段**，不改任何运行时代码。两条注意：① **`LICENSE` 取官方原文一字不改**——正文前后加料（如某些公司统一的开源前言）会让 GitHub 的 licensee 因相似度低于阈值而把仓库标成 `Other`，补充说明一律放 README。② **第三方署名尚未提供**：CLI 是 tsup 单文件产物（`noExternal: [/.*/]`），实测把 9 个依赖的代码内联进了产物（`@modelcontextprotocol/sdk` / `ajv` / `ajv-formats` / `commander` / `fast-deep-equal` / `fast-uri` / `json-schema-traverse` / `zod` / `zod-to-json-schema`，MIT 与 BSD-3-Clause / ISC），而产物里 `@license` / `Copyright` 零命中——这些包的许可证都在各自独立的 `LICENSE` 文件里、不是源码横幅，打包器读不到，esbuild 的 `legalComments` 对本项目无效。这些许可证要求分发时保留版权声明，当前未随产物提供，属已知且有意接受的缺口；若日后要补，做法是生成一份 `THIRD_PARTY_NOTICES.md` 并让安装脚本随 CLI 一并下载。全仓依赖均为宽松许可证，**无 GPL / AGPL / SSPL**，新增依赖若带传染性许可证需先评估 |
