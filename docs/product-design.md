...
环境（Environment）
 ...
 └── 变量（Variable）×N
      ...
      ├── 是否敏感（默认：敏感；见下）
      ├── 备注（这个变量的作用，AI 会读）
      ├── 授权名单（哪些用户可读取值，仅敏感变量需要）
      └── 无权限时是否可见（默认：可见）
```

- 环境示例：`公司内部服务`、`测试数据库`、`第三方 SaaS`；
- 环境有 Owner（创建者），Owner 和 Admin 可管理环境（编辑名称/备注、删除）与变量、授权；
- **非敏感变量**（§10 决策 23）：服务地址、端口这类公开配置可标记为非敏感——值明文存储（`value_plain`），全员免授权可读，清单直接附带明文值（控制台明文展示可复制、CLI/MCP 清单含 `value`），读取不落审计、不进密钥指纹清单。敏感性可在更新时切换。
...
- 敏感变量值加密存储（信封加密：主密钥 KEK 来自部署环境变量，数据密钥 AES-256-GCM）；非敏感变量明文存储、不适用本节审计要求（§10 决策 23）；
...
4. **分配结果自动生成为一个环境（或环境变量组）**：`DB_HOST / DB_PORT / DB_NAME / DB_USER / DB_PASSWORD`——其中**仅 `DB_PASSWORD` 为敏感**（加密存储、默认仅授权给申请人、读取落审计），主机/端口/库名/账号为非敏感明文（§10 决策 23）；数据库账号的下发、拉取、审计**完全复用环境变量体系**；
...
| 23 | 非敏感环境变量（明文存储） | 变量新增 **`secret` 标记（默认敏感）**：敏感值加密存储（`value_encrypted`）、读取需授权并落 `secret.read` 审计，行为不变；**非敏感值明文存储**（`value_plain` 列）、**全员免授权可读**——清单（`GET variables` / catalog / MCP `list_env_variables`）直接附带明文 `value`，控制台表格明文展示并可复制，读取不落审计、**不进密钥指纹清单**（不是密钥，出现在代码里不算泄露）。数据库分配生成的凭证环境同步遵循：**仅 `DB_PASSWORD` 敏感**，`DB_HOST/DB_PORT/DB_NAME/DB_USER` 为非敏感明文（§3.3、§3.7）。同批补齐控制台缺失的管理入口：环境详情页**编辑（名称/备注）/ 删除环境**，部署项目弹窗**编辑配置 / 删除项目**（接口原已具备）；数据库分配**删除后（status=deleted）不再出现在任何列表**，记录仅留库内供审计 |
