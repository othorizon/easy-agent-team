...
| `eat deploy [project]` | 触发部署（前置检查内置）；触发成功返回完整部署 ID |
| `eat project list / status / deployments` | 项目清单、最近一次部署状态（`--deployment <id>` 查指定那次，兼收完整 ID 与 8 位短 ID，决策 25）、部署历史 |
...
| `trigger_deploy` / `get_deploy_status` | 触发部署 / 查最近一次（或指定那次）的状态与失败原因 |
